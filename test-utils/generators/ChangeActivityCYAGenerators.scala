...
import models.changeActivity.AmountProduced.{ Large, None => NoneProduced, Small, enumerable }
...
object ChangeActivityCYAGenerators {
  ...
  val emptyUserAnswersForChangeActivity =
  ...
  case class ChangeActivityCYAUserAnswers(userAnswers: UserAnswers) extends TryValues {
        ...
        case Some(true) =>
          userAnswers
            ...
            .set(
              ...
              LitresInBands(ownBrandsLitresLowBand, ownBrandsLitresHighBand)
            )
            .success
            .value
      ...
      val userAnswersWithContract = contract match {
        case Some(true) =>
          userAnswers
            ...
            .success
            .value
            .set(HowManyContractPackingPage, LitresInBands(contractLitresLowBand, contractLitresHighBand))
            .success
            .value
      ...
      val userAnswersWithImport = imports match {
        case Some(true) =>
          userAnswers
            ...
            .success
            .value
            .set(HowManyImportsPage, LitresInBands(importLitresLowBand, importLitresHighBand))
            .success
            .value
    ...
    def withSites(packingSites: Option[Site] = None, warehouse: Option[Site] = None): ChangeActivityCYAUserAnswers = {
      val userAnswersWithSites = (packingSites, warehouse) match {
        case (Some(packingSite), Some(warehouseSite)) =>
          userAnswers
            ...
            .success
            .value
        case (Some(packingSite), None) =>
          userAnswers
            .copy(packagingSiteList = Map("1" -> packingSite))
            .set(PackagingSiteDetailsPage, true)
            .success
            .value
        case (None, Some(warehouseSite)) =>
          userAnswers
  ...
  def getUserAnswers(
    ...
    ChangeActivityCYAUserAnswers(
      emptyUserAnswersForChangeActivity.set(SecondaryWarehouseDetailsPage, false).success.value
    )
  ...
  val thirdPartyPackagingValues: Map[Answers.Value, (String, Option[Boolean])] =
    Map(
      Answers.Yes        -> ("using third party packagers", Some(true)),
      Answers.No         -> ("not using third party packagers", Some(false)),
      Answers.Unanswered -> ("", None)
    )
  val ownBrandsValues: Map[Answers.Value, (String, Option[Boolean])] =
    Map(
      Answers.Yes        -> ("producing own brands", Some(true)),
      Answers.No         -> ("not producing own brands", Some(false)),
      Answers.Unanswered -> ("", None)
    )
  val contractValues: Map[Answers.Value, (String, Option[Boolean])] =
    Map(
      Answers.Yes        -> ("contract packing", Some(true)),
      Answers.No         -> ("not contract packing", Some(false)),
      Answers.Unanswered -> ("", None)
    )
  val importValues: Map[Answers.Value, (String, Option[Boolean])] =
    Map(
      Answers.Yes        -> ("importing", Some(true)),
      Answers.No         -> ("not importing", Some(false)),
      Answers.Unanswered -> ("", None)
    )
  val packAtBusinessAddressValues: Map[Answers.Value, (String, Option[Boolean])] =
    Map(
      Answers.Yes        -> ("packing at business address", Some(true)),
      Answers.No         -> ("not packing at business address", Some(false)),
      Answers.Unanswered -> ("", None)
    )
  val packingSitesValues: Map[Answers.Value, (String, Option[Site])] =
    Map(
      Answers.Yes -> ("added a packing site" -> Some(
        Site(
          address = UkAddress(List("63 Clifton Roundabout", "Worcester"), "WR53 7CX"),
          ref = None,
          tradingName = Some("Test trading name 1"),
          closureDate = None
        )
      )),
      Answers.No -> ("had a packing site" -> Some(
        Site(
          address = UkAddress(List("63 Clifton Roundabout", "Worcester"), "WR53 7CX"),
          ref = None,
          tradingName = Some("Test trading name 1"),
  ...
  val warehouseValues: Map[Answers.Value, (String, Option[Site])] = Map(
    Answers.Yes -> ("added a warehouse site" -> Some(
      Site(
        address = UkAddress(List("63 Clifton Roundabout", "Worcester"), "WR53 7CX"),
        tradingName = Some("Test trading name 1")
      )
    )),
    Answers.No -> ("had a warehouse site" -> Some(
      Site(
        address = UkAddress(List("63 Clifton Roundabout", "Worcester"), "WR53 7CX"),
        tradingName = Some("Test trading name 1")
      )
  ...
  val largeTestCaseOptions: List[UserAnswerOptions] = Answers.All.flatMap { tpp =>
    Answers.Answered.map { imp =>
      List(
        UserAnswerOptions(
          amountProducedValues(APAnswers.Large),
          thirdPartyPackagingValues(tpp),
          ownBrandsValues(Answers.Yes),
          contractValues(Answers.Yes),
          importValues(imp),
          warehouseValues(Answers.Yes),
          packingSitesValues(Answers.Yes),
          packAtBusinessAddressValues(Answers.Yes)
        ),
        UserAnswerOptions(
          amountProducedValues(APAnswers.Large),
          thirdPartyPackagingValues(tpp),
          ...
          warehouseValues(Answers.Yes),
          packingSitesValues(Answers.Yes),
          packAtBusinessAddressValues(Answers.No)
        ),
        UserAnswerOptions(
          amountProducedValues(APAnswers.Large),
          thirdPartyPackagingValues(tpp),
          ownBrandsValues(Answers.Yes),
          contractValues(Answers.No),
          importValues(imp),
          warehouseValues(Answers.Yes),
          packingSitesValues(Answers.Yes),
          packAtBusinessAddressValues(Answers.Yes)
        ...
        UserAnswerOptions(
          ...
          warehouseValues(Answers.Yes),
          packingSitesValues(Answers.Yes),
          packAtBusinessAddressValues(Answers.Unanswered)
        )
      )
    }
  }.flatten
  ...
  val smallTestCaseNoTPPOptions: List[UserAnswerOptions] = Answers.Answered.flatMap { ob =>
    Answers.Answered.map { imp =>
      List(
        UserAnswerOptions(
          amountProducedValues(APAnswers.Small),
          thirdPartyPackagingValues(Answers.No),
          ownBrandsValues(ob),
          contractValues(Answers.Yes),
          importValues(imp),
          warehouseValues(Answers.Yes),
  ...
  val smallTestCaseWithTPPOptions: List[UserAnswerOptions] = Answers.Answered.flatMap { ob =>
    List(
      UserAnswerOptions(
        amountProducedValues(APAnswers.Small),
        thirdPartyPackagingValues(Answers.Yes),
        ownBrandsValues(ob),
        contractValues(Answers.Yes),
        importValues(Answers.Yes),
        warehouseValues(Answers.Yes),
        packingSitesValues(Answers.Yes),
        packAtBusinessAddressValues(Answers.Yes)
      ),
  ...
  val smallTestCaseVoluntaryOptions: List[UserAnswerOptions] = Answers.Answered.flatMap { ob =>
    List(
      UserAnswerOptions(
        ...
        contractValues(Answers.No),
        importValues(Answers.No),
        warehouseValues(Answers.Unanswered),
        packingSitesValues(Answers.Unanswered),
        packAtBusinessAddressValues(Answers.Unanswered)
      )
    )
  }

  val noneTestCaseOptions: List[UserAnswerOptions] = Answers.All
    .flatMap { tpp =>
      Answers.All.map { ob =>
        Answers.Answered.map { imp =>
          List(
            UserAnswerOptions(
              amountProducedValues(APAnswers.NoneProduced),
              thirdPartyPackagingValues(tpp),
              ownBrandsValues(ob),
              contractValues(Answers.Yes),
              importValues(imp),
