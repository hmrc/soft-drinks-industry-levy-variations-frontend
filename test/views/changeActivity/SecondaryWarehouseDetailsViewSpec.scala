...
class SecondaryWarehouseDetailsViewSpec extends ViewSpecHelper {
  ...
  "View" - {
    ...
    "should contain the expected title" in {
      document.title() must include(Messages("changeActivity.secondaryWarehouseDetails" + ".title"))
    }

    "should include a legend with the expected heading" in {
      val legend = document.getElementsByClass(Selectors.legend)
      legend.size() mustBe 1
      legend.get(0).getElementsByClass(Selectors.heading).text() mustEqual Messages("changeActivity.secondaryWarehouseDetails.subHeading")
    }
    ...
    "when there are form errors" - {
      ...
      "should have a title containing error" in {
        val titleMessage = Messages("changeActivity.secondaryWarehouseDetails.title")
