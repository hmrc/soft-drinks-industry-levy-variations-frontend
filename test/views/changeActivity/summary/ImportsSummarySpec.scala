...
import views.summary.changeActivity.ImportsSummary
...
class ImportsSummarySpec extends SpecBase {
  ...
  "summaryList" - {
    ...
    "should return correct elements when passed in with TRUE and litres provided and check answers is true" in {
      ...
      val res = ImportsSummary.summaryList(userAnswers, isCheckAnswers = true, includeLevyRows = false)
      res.rows.head.key.content.asHtml mustBe Html("Do you bring liable drinks into the UK?")
      ...
      res.rows.head.actions.head.items.head.href mustBe "/change-imports"
...
//      res.rows(1).value.content.asHtml mustBe Html("1,000")
      ...
      res.rows(1).actions.head.items.head.href mustBe "/change-how-many-imports-next-12-months"
...
//      res.rows(2).value.content.asHtml mustBe Html("2,000")
      ...
      res.rows(2).actions.head.items.head.href mustBe "/change-how-many-imports-next-12-months"
      res.rows(2).actions.head.items.head.attributes mustBe Map("id" -> "change-highband-litreage-imports")
    ...
    "should return correct elements when passed in with TRUE and litres provided and check answers is false" in {
      ...
      val res = ImportsSummary.summaryList(userAnswers, isCheckAnswers = false, includeLevyRows = false)
      res.rows.head.key.content.asHtml mustBe Html("Do you bring liable drinks into the UK?")
      res.rows.head.key.classes mustBe ""
      res.rows.head.value.content.asHtml mustBe Html("Yes")
      res.rows.head.value.classes.trim mustBe "govuk-!-text-align-right"
