...
class RemoveWarehouseDetailsViewSpec extends ViewSpecHelper {
  ...
  "View" - {
    ...
    val document = doc(view(form, html, index, NormalMode)(using request, messages(application)))
    ...
    "when the form is preoccupied with yes and has no errors" - {
      val html1 = view(form.fill(true), html, index, NormalMode)(using request, messages(application))
    ...
    "when the form is preoccupied with no and has no errors" - {
      val html1 = view(form.fill(false), html, index, NormalMode)(using request, messages(application))
      val document1 = doc(html1)
      "should have radio buttons" - {
        val radioButtons = document1.getElementsByClass(Selectors.radios)
        "that has the option to select Yes and is unchecked" in {
    ...
    "contains a form with the correct action" - {
      val htmlYesSelected = view(form.fill(true), html, index, NormalMode)(using request, messages(application))
      val documentYesSelected = doc(htmlYesSelected)

      val htmlNoSelected = view(form.fill(false), html, index, NormalMode)(using request, messages(application))
    ...
    "when there are form errors" - {
      val htmlWithErrors =
        view(form.bind(Map("value" -> "")), html, index, NormalMode)(using request, messages(application))
