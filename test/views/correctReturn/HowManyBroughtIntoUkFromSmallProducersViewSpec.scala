...
class HowManyBroughtIntoUkFromSmallProducersViewSpec extends LitresSpecHelper {
  ...
  val howManyBroughtIntoUkFromSmallProducersView: HowManyBroughtIntoUkFromSmallProducersView =
    application.injector.instanceOf[HowManyBroughtIntoUkFromSmallProducersView]
    ...
    List(NormalMode, CheckMode).foreach { mode =>
      "when in " + mode + " mode" - {
        ...
        val htmlWithValidData: HtmlFormat.Appendable =
          howManyBroughtIntoUkFromSmallProducersView(formWithHighAndLowBands, mode)
        ...
        "and the form has errors" - {
          val errorTitle =
            "Error: How many litres of liable drinks have you brought into the UK from small producers? - Soft Drinks Industry Levy - GOV.UK"
