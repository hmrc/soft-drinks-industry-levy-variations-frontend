...
class ReturnChangeRegistrationViewSpec extends ViewSpecHelper {
  ...
  "View" - {
    val html = view(NormalMode, "")(using request, messages(application))
    ...
    "should have the expected heading" in {
      document.getElementsByClass(Selectors.heading).text() mustEqual Messages(
        "You changed your soft drinks business activity"
      )
    }
    ...
    "should render correctly with Packaged Contract Packer link when coPacker is false but coPacker data was completed" in {
      ...
      bodyText.get(
        1
      ) mustBe "This is different to your registered business activity, so we will update your registration."
      ...
      linkInBodyText.attr(
        "href"
      ) mustBe "/soft-drinks-industry-levy-variations-frontend/correct-return/packaged-as-contract-packer"
    }
