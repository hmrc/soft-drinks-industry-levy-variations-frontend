  ...
  "summaryList" - {
    ...
    returnPeriods.foreach(returnPeriod => {
      includeLevyRowsOptions.foreach(includeLevyRows => {
        s"should return correct elements when passed in with TRUE and litres provided and check answers is true and include levy rows $includeLevyRows ${returnPeriod._2}" in {
        ...
        s"should return correct elements when passed in with TRUE and litres provided and check answers is false and include levy rows $includeLevyRows ${returnPeriod._2}" in {
          val userAnswers = emptyUserAnswersForCorrectReturn.copy(correctReturnPeriod = Some(returnPeriod._1))
            .set(OperatePackagingSiteOwnBrandsPage, true).success.value
            .set(HowManyOperatePackagingSiteOwnBrandsPage, LitresInBands(lowLitres, highLitres)).success.value
        ...
        s"should return correct elements when passed in with FALSE and NO litres provided and include levy rows $includeLevyRows ${returnPeriod._2}" in {
          val userAnswers = emptyUserAnswersForCorrectReturn.copy(correctReturnPeriod = Some(returnPeriod._1))
        ...
        s"should return correct elements when no elements provided and include levy rows $includeLevyRows ${returnPeriod._2}" in {
          val userAnswers = emptyUserAnswersForCorrectReturn.copy(correctReturnPeriod = Some(returnPeriod._1))
