  ...
  "ClaimCreditsForLostDamagedSummary" - {
    ...
    returnPeriodWithLabels.foreach { returnPeriod =>
      s"must show correct rows when business has Lost/Damaged is true, litres provided, and checkAnswers is true ${returnPeriod._2}" in {
        ...
        res.rows.head.actions.head.items.head.href mustBe routes.ClaimCreditsForLostDamagedController
        ...
        res.rows(1).actions.head.items.head.href mustBe routes.HowManyCreditsForLostDamagedController
          .onPageLoad(CheckMode)
          .url
        res.rows(1).actions.head.items.head.attributes mustBe Map(
          "id" -> "change-lowband-litreage-correctReturn.claimCreditsForLostDamaged"
        )
        ...
        res.rows(3).actions.head.items.head.href mustBe routes.HowManyCreditsForLostDamagedController
          .onPageLoad(CheckMode)
          .url
        res.rows(3).actions.head.items.head.attributes mustBe Map(
          "id" -> "change-highband-litreage-correctReturn.claimCreditsForLostDamaged"
        )
      ...
      s"should return correct elements when business has Lost/Damaged is false and NO litres provided  ${returnPeriod._2}" in {
        ...
        res.rows.head.value.classes.trim mustBe "sdil-right-align--desktop"
        res.rows.head.actions.head.items.head.href mustBe controllers.correctReturn.routes.ClaimCreditsForLostDamagedController
          .onPageLoad(CheckMode)
          .url
        res.rows.head.actions.head.items.head.attributes mustBe Map("id" -> "change-claimCreditsForLostDamaged")
        res.rows.head.actions.head.items.head.content.asHtml mustBe Html("Change")
