...
class PackagedAsContractPackerSummarySpec extends SpecBase {
  ...
  "PackagedAsContractPackerSummary" - {
    ...
    "must show Yes with litres and levy rows when reporting contract packed" in {
      ...
      val res = PackagedAsContractPackerSummary
    ...
    "must show No when not reporting contract packed" in {
      val userAnswers = emptyUserAnswersForCorrectReturn
