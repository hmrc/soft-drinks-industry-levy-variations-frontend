...
class WarehouseDetailsSummarySpec extends SpecBase {
  ...
  "row" - {
    ...
    "should return a summary list row with the correct information and action links" in {
      ...
      val warehouseDetailsSummaryRow = WarehouseDetailsSummary.row2(warehouses2, NormalMode)
  ...
  "row should contain the same number of rows as the number of small producers" - {
    ...
    "should have 1 row" in {
      val warehouseDetailsSummaryRow = WarehouseDetailsSummary.row2(warehouses1, NormalMode)

      warehouseDetailsSummaryRow.size mustBe 1
    }

    "should have 3 rows" in {
      val warehouseDetailsSummaryRow = WarehouseDetailsSummary.row2(warehouses3, NormalMode)
