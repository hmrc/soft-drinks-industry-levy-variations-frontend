...
class WarehouseDetailsViewSpec extends ViewSpecHelper {
  ...
  "View" - {
    ...
    "when the form is preoccupied with yes and has no errors" - {
      val html1 = view(form.fill(true), NormalMode, Some(SummaryList()))(using request, messages(application))
    ...
    "when the form is preoccupied with no and has no errors" - {
      val html1 = view(form.fill(false), NormalMode, Some(SummaryList()))(using request, messages(application))
      val document1 = doc(html1)
      "should have radio buttons" - {
        val radioButtons = document1.getElementsByClass(Selectors.radios)
        "that has the option to select Yes and is unchecked" in {
    ...
    "contains a form with the correct action" - {
      "when in CheckMode" - {
        val htmlYesSelected =
          view(form.fill(true), CheckMode, Some(SummaryList()))(using request, messages(application))
        val documentYesSelected = doc(htmlYesSelected)

        val htmlNoSelected =
          view(form.fill(false), CheckMode, Some(SummaryList()))(using request, messages(application))
      ...
      "when in NormalMode" - {
        val htmlYesSelected =
          view(form.fill(true), NormalMode, Some(SummaryList()))(using request, messages(application))
        val documentYesSelected = doc(htmlYesSelected)

        val htmlNoSelected =
          view(form.fill(false), NormalMode, Some(SummaryList()))(using request, messages(application))
        val documentNoSelected = doc(htmlNoSelected)
        "and yes is selected" in {
          documentYesSelected
            .select(Selectors.form)
    ...
    "when there are form errors" - {
      val htmlWithErrors =
        view(form.bind(Map("value" -> "")), NormalMode, Some(SummaryList()))(using request, messages(application))
