...
class ReasonControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val reasonRoute: String = routes.ReasonController.onPageLoad(NormalMode).url
  ...
  "Reason Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, reasonRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = emptyUserAnswersForCancelRegistration.set(ReasonPage, "answer").success.value
    ...
    "must fail if the setting of userAnswers fails" in {
      ...
      val application =
        applicationBuilder(userAnswers = Some(userDetailsWithSetMethodsReturningFailure(CancelRegistration))).build()
        ...
        withCaptureOfLoggingFrom(application.injector.instanceOf[GenericLogger].logger) { events =>
          ...
          events
            .collectFirst { case event =>
              ...
              event.getMessage mustEqual "Failed to set value in session repository while attempting set on reason"
            }
