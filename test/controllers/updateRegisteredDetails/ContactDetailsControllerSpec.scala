...
class ContactDetailsControllerSpec extends SpecBase {
  ...
  lazy val contactDetailsRoute = routes.ContactDetailsController.onPageLoad().url
  ...
  "ContactDetails Controller" - {
    ...
    "User should be redirected to an error page if setting of the contact details in session (user answers) fails" in {

      val mockSessionService = mock[SessionService]

      when(mockSessionService.set(any())) thenReturn Future.successful(Left(SessionDatabaseInsertError))

      val application = applicationBuilder(userAnswers = Some(emptyUserAnswersForUpdateRegisteredDetails)).overrides(
