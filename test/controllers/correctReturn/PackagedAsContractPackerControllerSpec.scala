...
class PackagedAsContractPackerControllerSpec extends SpecBase with MockitoSugar {
  ...
  "PackagedAsContractPacker Controller" - {
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, packagedAsContractPackerRoute)
        ...
        withCaptureOfLoggingFrom(application.injector.instanceOf[GenericLogger].logger) { events =>
          ...
          events.collectFirst {
            case event =>
              ...
              event.getMessage mustEqual "Failed to set value in session repository while attempting set on packagedAsContractPacker"
