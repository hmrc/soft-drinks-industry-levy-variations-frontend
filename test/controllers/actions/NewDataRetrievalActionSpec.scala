...
class NewDataRetrievalActionSpec extends SpecBase with MockitoSugar {

  class Harness(sessionRepository: SessionRepository, sdilSessionCache: SDILSessionCache) extends NewDataRetrievalActionImpl(sessionRepository, sdilSessionCache) {
  ...
  "Data Retrieval Action" - {
    ...
    "when there is data in the cache" - {
      ...
      "must build a userAnswers object and add it to the request" in {
        ...
        val sessionRepository = mock[SessionRepository]
        val sdilSessionCache = mock[SDILSessionCache]
        when(sessionRepository.get("id")) thenReturn Future(Some(UserAnswers("id", journeyType = SelectChange.ChangeActivity)))
        when(sdilSessionCache.fetchEntry[ReturnPeriod]("id", SDILSessionKeys.RETURN_PERIOD)) thenReturn Future(Some(returnPeriod.head))
        val action = new Harness(sessionRepository, sdilSessionCache)
