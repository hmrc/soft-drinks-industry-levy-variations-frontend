...
class RequiredUserAnswersForCorrectReturnSpec extends SpecBase with DefaultAwaitTimeout {
  ...
  val requiredUserAnswers: RequiredUserAnswersForCorrectReturn =
    application.injector.instanceOf[RequiredUserAnswersForCorrectReturn]
  ...
  "requireData - CYA" - {
    ...
    "should return redirect to Packaged as Contract Packer page when user answers is empty and the small producer is true" in {
      val res = requiredUserAnswers.requireData(CorrectReturnBaseCYAPage, basicUserAnswers, basicSubscription)(
    ...
    "should return Redirect to Own brands page when user answers is empty and the small producer is false" in {
      ...
      val res = requiredUserAnswers.requireData(CorrectReturnBaseCYAPage, basicUserAnswers, subscription)(
        Future.successful(Ok(""))
      )
    ...
    "should allow user to continue if all user answers are filled in and user is NOT newImporter && NOT co packer && NOT small producer" in {
      val completedUserAnswers = emptyUserAnswersForCorrectReturn
        ...
        .success
        .value
        .set(HowManyPackagedAsContractPackerPage, LitresInBands(100, 652))
        .success
        .value
      ...
      val res = requiredUserAnswers.requireData(CorrectReturnBaseCYAPage, completedUserAnswers, basicSubscription)(
        Future.successful(Ok(""))
      )
  ...
  "requireData" - {
    "should take a random page and allow user to carry on their action with empty answers" in {
      val res = requiredUserAnswers.requireData(OperatePackagingSiteOwnBrandsPage, basicUserAnswers, basicSubscription)(
        Future.successful(Ok(""))
      )
      status(res) mustBe OK
    }
    s"should check for $CorrectReturnBaseCYAPage and redirect to first missing page when answers incomplete but not a " +
      s"nil return and small producer is false" in {
        ...
        val res = requiredUserAnswers.requireData(
          CorrectReturnBaseCYAPage,
          emptyUserAnswersForCorrectReturn.copy(data = Json.obj("foo" -> "bar")),
          subscription
        )(Future.successful(Ok("")))
        redirectLocation(res).get mustBe routes.OperatePackagingSiteOwnBrandsController.onPageLoad(CheckMode).url
      }
    ...
    s"should check for $CorrectReturnBaseCYAPage and allow the user to continue when all answers are complete" in {
      ...
      val res =
        requiredUserAnswers.requireData(CorrectReturnBaseCYAPage, completedUserAnswers, basicSubscription)(
          Future.successful(Ok(""))
        )
      status(res) mustBe OK
    }
  ...
  "requireData - CheckChanges" - {
    "should return Redirect to CYA page when user answers past check your answers not submitted" in {
      ...
      val res = requiredUserAnswers.requireData(CorrectReturnCheckChangesPage, userAnswers, basicSubscription)(
        Future.successful(Ok("foo"))
      )
    ...
    "should return Redirect to Repayment reason page if it is not filled in and balance repayment required" in {
      val completedUserAnswers = emptyUserAnswersForCorrectReturn
        ...
        .success
        .value
        .set(CorrectionReasonPage, "some info")
        .success
        .value
      val res = requiredUserAnswers.requireData(CorrectReturnCheckChangesPage, completedUserAnswers, basicSubscription)(
        Future.successful(Ok("foo"))
      )
  ...
  "requireData - CorrectionReason" - {
    "should return Redirect to CYA page when user answers past check your answers not submitted" in {
      val userAnswers = basicUserAnswers
      val res = requiredUserAnswers.requireData(CorrectionReasonPage, userAnswers, basicSubscription)(
        Future.successful(Ok("foo"))
      )
      redirectLocation(res).get mustBe routes.CorrectReturnCYAController.onPageLoad.url
    }
  ...
  "requireData - RepaymentMethod" - {
    "should return Redirect to CYA page when user answers past check your answers not submitted" in {
      val userAnswers = basicUserAnswers
      val res = requiredUserAnswers.requireData(RepaymentMethodPage, userAnswers, basicSubscription)(
        Future.successful(Ok("foo"))
      )
      redirectLocation(res).get mustBe routes.CorrectReturnCYAController.onPageLoad.url
    }
  ...
  "returnMissingAnswers" - {
    "should return all missing answers in a list when user answers is empty" in {
      val res = requiredUserAnswers.returnMissingAnswers(
        basicUserAnswers,
        basicSubscription,
        CorrectReturnBaseCYAPage.previousPagesRequired
      )
      res mustBe List(
  ...
  "mainRoute" - {
    ...
    "should return all correct answers if user is newImporter && newCopacker && small producer true" in {
      val completedUserAnswers = emptyUserAnswersForCorrectReturn
        ...
        .success
        .value
        .set(HowManyBroughtIntoUKPage, LitresInBands(259, 923))
        .success
        .value
