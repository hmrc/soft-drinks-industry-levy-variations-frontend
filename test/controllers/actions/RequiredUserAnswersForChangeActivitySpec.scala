...
class RequiredUserAnswersForChangeActivitySpec extends SpecBase with DefaultAwaitTimeout {
  ...
  val requiredUserAnswers: RequiredUserAnswersForChangeActivity =
    application.injector.instanceOf[RequiredUserAnswersForChangeActivity]
  ...
  val basicUserAnswers = emptyUserAnswersForChangeActivity
  val basicSubscription = aSubscription

  "requireData" - {
    "should return result passed in when not a page matched in function" in {
      contentAsString(
        requiredUserAnswers.requireData(AmountProducedPage, basicUserAnswers, basicSubscription)(exampleSuccessAction)
      ...
      s"when AmountProduced is $Large and all answers are answered" in {
        val userAnswers =
          emptyUserAnswersForChangeActivity
            ...
            .value
        contentAsString(
          requiredUserAnswers.requireData(ChangeActivityCYAPage, userAnswers, basicSubscription)(exampleSuccessAction)
        ) mustBe exampleSuccessActionResult
      }
    }
          ...
          s"${AmountProduced.Large}" in {
            val userAnswers =
              emptyUserAnswersForChangeActivity
                ...
                .value
            val res = redirectLocation(
              requiredUserAnswers.requireData(ChangeActivityCYAPage, userAnswers, basicSubscription)(
  ...
  "returnMissingAnswers" - {
    List(true, false).foreach { packagingSitesEmpty =>
      ...
      s"should return all missing answers when user answers is empty and packaging site list is ${
          if (packagingSitesEmpty) "" else "not "
        }empty" in {
        val userAnswers = emptyUserAnswersForChangeActivity.copy(packagingSiteList = packagingSiteList)
        val res = requiredUserAnswers.returnMissingAnswers(
          userAnswers,
          basicSubscription,
          ChangeActivityCYAPage.previousPagesRequired
        )
      ...
      s"should return correct missing answer list when producer is $Large, contractPacking is false, OperatePackagingSites " +
        s"is true and pack at business address is not answered and packaging site list is ${
            if (packagingSitesEmpty) "" else "not "
          }empty" in {
          val userAnswers =
            emptyUserAnswersForChangeActivity
      ...
      s"should return correct missing answer list when producer is $Large, $OperatePackagingSiteOwnBrandsPage " +
        s"is true and PackAtBusinessAddress is not answered and packaging site list is ${
            if (packagingSitesEmpty) "" else "not "
          }empty" in {
          val userAnswers =
            emptyUserAnswersForChangeActivity
      ...
      s"should return correct missing answer list when producer is $Small, $ContractPackingPage is true, $OperatePackagingSiteOwnBrandsPage " +
        s"is true, and pack at business address is not answered and packaging site list ${
            if (packagingSitesEmpty) "" else "not "
          }empty" in {
          val userAnswers =
            emptyUserAnswersForChangeActivity
  ...
  s"ThirdPartyPackagerRequiredData" - {
    s"should return empty missing answer list when producer is $Small" in {
      ...
      val res = requiredUserAnswers.returnMissingAnswers(
        userAnswers,
        basicSubscription,
        ThirdPartyPackagersPage.previousPagesRequired
      )
  ...
  s"OperatePackagingSiteRequiredData" - {
        ...
        s" redirect to $ThirdPartyPackagersPage" in {
          val userAnswers =
            emptyUserAnswersForChangeActivity
              ...
              .value
          val res = redirectLocation(
            requiredUserAnswers.requireData(OperatePackagingSiteOwnBrandsPage, userAnswers, basicSubscription)(
              exampleSuccessAction
            )
          )
    ...
    s"should return correct missing answer list when producer is $Small" in {
      ...
      val res = requiredUserAnswers.returnMissingAnswers(
        userAnswers,
        basicSubscription,
        OperatePackagingSiteOwnBrandsPage.previousPagesRequired
      )
  ...
  "ContractPackingPageRequiredData" - {
        ...
        s" redirect to $ThirdPartyPackagersPage" in {
          val userAnswers =
            emptyUserAnswersForChangeActivity
              ...
              .value
          val res = redirectLocation(
            requiredUserAnswers.requireData(ContractPackingPage, userAnswers, basicSubscription)(exampleSuccessAction)
          )
          res.get mustBe controllers.changeActivity.routes.ThirdPartyPackagersController.onPageLoad(NormalMode).url
        }
    }
    ...
    s"should return correct missing answer list when producer is $Small and no other answers have been set" in {
      ...
      val res = requiredUserAnswers.returnMissingAnswers(
        userAnswers,
        basicSubscription,
        ContractPackingPage.previousPagesRequired
      )
    ...
    s"should redirect to the appropriate missing page when missing answers required for $ImportsPage" - {
        ...
        s" redirect to $ThirdPartyPackagersPage" in {
          val userAnswers =
            emptyUserAnswersForChangeActivity
              ...
              .value
          val res = redirectLocation(
            requiredUserAnswers.requireData(ImportsPage, userAnswers, basicSubscription)(exampleSuccessAction)
          )
          res.get mustBe controllers.changeActivity.routes.ThirdPartyPackagersController.onPageLoad(NormalMode).url
        }
      ...
      s"when AmountProduced is $Small and $ThirdPartyPackagersPage has been completed, on page load of $ImportsPage should" +
        s" redirect to $OperatePackagingSiteOwnBrandsPage" in {
          ...
          res.get mustBe controllers.changeActivity.routes.OperatePackagingSiteOwnBrandsController
    ...
    s"should return correct missing answer list when producer is $Small and no other answers have been set" in {
      val userAnswers =
        emptyUserAnswersForChangeActivity
          ...
          .value
      val res =
        requiredUserAnswers.returnMissingAnswers(userAnswers, basicSubscription, ImportsPage.previousPagesRequired)
