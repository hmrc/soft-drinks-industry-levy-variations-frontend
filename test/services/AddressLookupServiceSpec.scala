...
class AddressLookupServiceSpec extends SpecBase with FutureAwaits with DefaultAwaitTimeout {
  ...
  "addAddressUserAnswers" - {
    s"add to the cache the address of a $WarehouseDetails when a user returns from address lookup frontend where sdilId DOESN'T exist" in {
      ...
      val warehouseMap =
        Map("1" -> Site(UkAddress(List("33 Rhes Priordy", "East London"), "E73 2RP"), Some("super cola")))
    ...
    s"add to the cache the address of a $PackingDetails when a user returns from address lookup frontend with missing address lines" in {
      ...
      val addedPackingSiteMissingLines = Map(
        ...
        sdilId -> Site(
          UkAddress(List(addressLine1, addressLine2), postcode, alfId = Some(alfId)),
          None,
          Some(organisation),
          None
        )
    ...
    s"add to the cache the address of a $ContactDetails when a user returns from address lookup frontend with full address lines" in {
      ...
      val alfId: String = "bar"
      val contactAddress = UkAddress(List("33 Rhes Priordy", "East London"), "E73 2RP")
      val updatedContactAddress =
        UkAddress(List(addressLine1, addressLine2, addressLine3, addressLine4), postcode, Some(alfId))
  ...
  List(NormalMode, CheckMode).foreach { mode =>
      ...
      s"should return Successful future when connector returns success for $PackingDetails" in {
        ...
        val expectedJourneyConfigToBePassedToConnector = JourneyConfig(
          ...
          options = JourneyOptions(
            continueUrl = s"http://localhost:8705/soft-drinks-industry-levy-variations-frontend/off-ramp/${
                if (mode == CheckMode) "change-" else ""
            ...
            phaseFeedbackLink = Some(
              s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-variations-frontend&backUrl=http%3A%2F%2Flocalhost%3A8705bar"
            ),
    ...
    s"createJourneyConfig in $mode" - {
      s"should return a journey config for $WarehouseDetails" in {
        ...
        val expected = JourneyConfig(
          ...
          options = JourneyOptions(
            continueUrl = s"http://localhost:8705/soft-drinks-industry-levy-variations-frontend/off-ramp/${
                if (mode == CheckMode) "change-" else ""
              }secondary-warehouses/$exampleSdilIdWeGenerate",
            homeNavHref = None,
            signOutHref = Some(controllers.auth.routes.AuthController.signOut().url),
            accessibilityFooterUrl =
              Some("localhost/accessibility-statement/soft-drinks-industry-levy-variations-frontend"),
            phaseFeedbackLink = Some(
              s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-variations-frontend&backUrl=http%3A%2F%2Flocalhost%3A8705${request.uri}"
            ),
            deskProServiceName = None,
            showPhaseBanner = Some(false),
            alphaPhase = Some(frontendAppConfig.AddressLookupConfig.alphaPhase),
      ...
      s"should return a journey config for $PackingDetails" in {
        val request = FakeRequest("foo", "bar")
        val exampleSdilIdWeGenerate: String = "wizz"
        val res = service.createJourneyConfig(PackingDetails, exampleSdilIdWeGenerate, mode)(using request, implicitly)
        val expected = JourneyConfig(
          version = 2,
          options = JourneyOptions(
            continueUrl = s"http://localhost:8705/soft-drinks-industry-levy-variations-frontend/off-ramp/${
      ...
      s"should return a journey config for $ContactDetails" in {
        val request = FakeRequest("foo", "bar")
        val exampleSdilIdWeGenerate: String = "wizz"
        val res = service.createJourneyConfig(ContactDetails, exampleSdilIdWeGenerate, mode)(using request, implicitly)
        val expected = JourneyConfig(
          version = 2,
          options = JourneyOptions(
            continueUrl = s"http://localhost:8705/soft-drinks-industry-levy-variations-frontend/off-ramp/${
                if (mode == CheckMode) "change-" else ""
              }business-address/$exampleSdilIdWeGenerate",
            homeNavHref = None,
            signOutHref = Some(controllers.auth.routes.AuthController.signOut().url),
            accessibilityFooterUrl =
              Some("localhost/accessibility-statement/soft-drinks-industry-levy-variations-frontend"),
