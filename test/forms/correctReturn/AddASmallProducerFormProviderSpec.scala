...
class AddASmallProducerFormProviderSpec extends LongFieldBehaviour with StringFieldBehaviours with SDILReferenceFieldBehaviours with SpecBase with MockitoSugar {
  ...
  lazy val correctReturn.addASmallProducerRoute = routes.AddASmallProducerController.onPageLoad(NormalMode).url

  ".producerName" - {

    val fieldName = "producerName"
    val lengthKey = "correctReturn.addASmallProducer.error.producerName.maxLength"
    val maxLength = 160
