...
class ChangedPageSpec extends AnyFreeSpec with Matchers {
  "returnLiteragePagesThatChangedComparedToOriginalReturn" - {
    "should return all pages with answers changed true that have changed if all answers have changed" in {
      val originalSdilReturn = SdilReturn(
        Litreage(0, 0),
        Litreage(0, 0),
        List(),
        Litreage(0, 0),
      ...
      val currentSdilReturn = SdilReturn(
        Litreage(1, 1),
        Litreage(3, 4),
      ...
      val res =
        ChangedPage.returnLiteragePagesThatChangedComparedToOriginalReturn(originalSdilReturn, currentSdilReturn)
      res mustBe List(
