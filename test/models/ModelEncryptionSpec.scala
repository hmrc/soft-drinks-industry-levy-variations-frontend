...
class ModelEncryptionSpec extends SpecBase {
  ...
  "encryptUserAnswers" - {
    "should encrypt userAnswers" in {
      ...
      Json
        .parse(encryption.crypto.decrypt(result._4, userAnswers.id))
        .as[List[SmallProducer]] mustBe userAnswers.smallProducerList
      Json
      ...
      Json
        .parse(encryption.crypto.decrypt(result._6.head._2, userAnswers.id))
        .as[Site] mustBe userAnswers.warehouseList.head._2
