...
object SpecBase {
  ...
  val userAnswerTwoWarehouses: UserAnswers = UserAnswers(
    sdilNumber,
  ...
  val userAnswerTwoWarehousesUpdateRegisteredDetails: UserAnswers = UserAnswers(
    sdilNumber,
  ...
  val voluntarySubscription: RetrievedSubscription = aSubscription.copy(activity =
    RetrievedActivity(
      smallProducer = false,
      largeProducer = true,
      contractPacker = false,
...
trait SpecBase
    ...
    with BeforeAndAfterEach with LoggerHelper with TestData {
  ...
  def testInvalidJourneyType(expectedJourneyType: SelectChange, url: String, hasPostMethod: Boolean = true): Unit = {
    val journeyTypesNotSupported = expectedJourneyType match {
      case SelectChange.CancelRegistration =>
        SelectChange.values.filter(!List(expectedJourneyType, SelectChange.ChangeActivity).contains(_))
  ...
  def userDetailsWithSetMethodsReturningFailure(selectChange: SelectChange): UserAnswers =
    new UserAnswers(
      ...
      selectChange,
      contactAddress = contactAddress,
      correctReturnPeriod = if (selectChange == SelectChange.CorrectReturn) { returnPeriod.headOption }
      else None
    ) {
