...
trait TestData {
  ...
  val amounts: Amounts = Amounts(
    originalReturnTotal = 1525.32,
    newReturnTotal = 1320.00,
    balanceBroughtForward = -502.75,
    totalForQuarterLessForwardBalance = 1822.75,
    netAdjustedAmount = 297.43
  )
  val cyaAmounts: Amounts = Amounts(
    originalReturnTotal = 0.00,
    newReturnTotal = -66.00,
    balanceBroughtForward = -500.00,
    totalForQuarterLessForwardBalance = 434.00,
    netAdjustedAmount = 434.00
  )
  ...
  val smallProducerList: List[SmallProducer] = List(
    SmallProducer(producerNameParty, sdilReferenceParty, Litreage(litres, litres))
  )
  ...
  val returnPeriodsFor2020: List[ReturnPeriod] =
    List(ReturnPeriod(2020, 3), ReturnPeriod(2020, 2), ReturnPeriod(2020, 1), ReturnPeriod(2020, 0))
  val returnPeriodsFor2022: List[ReturnPeriod] =
    List(ReturnPeriod(2022, 3), ReturnPeriod(2022, 2), ReturnPeriod(2022, 1), ReturnPeriod(2022, 0))

  val returnPeriodList: List[ReturnPeriod] = returnPeriodsFor2022 ++ returnPeriodsFor2020

  val JsonreturnPeriodList: List[JsValue] = List(
    Json.toJson(ReturnPeriod(2020, 0)),
    Json.toJson(ReturnPeriod(2020, 1)),
    Json.toJson(ReturnPeriod(2020, 2)),
    Json.toJson(ReturnPeriod(2020, 3)),
    Json.toJson(ReturnPeriod(2022, 0)),
    Json.toJson(ReturnPeriod(2022, 1)),
    Json.toJson(ReturnPeriod(2022, 2)),
    Json.toJson(ReturnPeriod(2022, 3))
  )
  ...
  val userAnswersWithUpdatedContactForUpdateRegisteredDetails: UserAnswers = UserAnswers(
    sdilNumber,
  ...
  val emptyUserAnswersForChangeActivity: UserAnswers =
    UserAnswers(sdilNumber, SelectChange.ChangeActivity, contactAddress = contactAddress)
  ...
  val expectedCorrectReturnDataForNilReturn: CorrectReturnUserAnswersData = CorrectReturnUserAnswersData(
    operatePackagingSiteOwnBrands = false,
    None,
    packagedAsContractPacker = false,
    None,
  ...
  val emptyUserAnswersForCorrectReturn: UserAnswers = UserAnswers(
    sdilNumber,
  ...
  def userAnswersForCorrectReturn(sdilReturnIsNil: Boolean): UserAnswers = {
    ...
    emptyUserAnswersForCorrectReturn
      .setForCorrectReturn(correctReturnData, smallProducers, returnPeriod.head)
      .success
      .value
  }

  val emptySdilReturn: SdilReturn = SdilReturn(
    Litreage(),
    Litreage(),
    List.empty,
    Litreage(),
  ...
  val userAnswersForCorrectReturnWithEmptySdilReturn: UserAnswers = userAnswersForCorrectReturn(true)
  val userAnswerTwoWarehouses: UserAnswers = userAnswersForCorrectReturn(true).copy(warehouseList = twoWarehouses)
  val emptyUserAnswersForCancelRegistration: UserAnswers =
    UserAnswers(sdilNumber, SelectChange.CancelRegistration, contactAddress = contactAddress)

  def emptyUserAnswersForSelectChange(selectChange: SelectChange): UserAnswers =
    UserAnswers(sdilNumber, selectChange, contactAddress = contactAddress)

  val changedUserSdilNumber = "XKSDIL000000023"
  val updatedSubscriptionWithChangedActivityToNewImporterAndPacker: RetrievedSubscription = aSubscription.copy(
    utr = "0000000023",
    sdilRef = "XKSDIL000000023",
    activity = RetrievedActivity(
      smallProducer = false,
      largeProducer = true,
      contractPacker = true,
  ...
  def completedUserAnswersForCorrectReturnNewPackerOrImporter: UserAnswers =
    userAnswersForCorrectReturnWithEmptySdilReturn
      .copy(
        id = changedUserSdilNumber,
