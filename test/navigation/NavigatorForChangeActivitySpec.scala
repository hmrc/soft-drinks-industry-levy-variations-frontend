...
class NavigatorForChangeActivitySpec extends SpecBase {
  ...
  "Navigator" - {
    ...
    "in Normal mode" - {
      ...
      "must go from a page that doesn't exist in the route map to Index" in {
        ...
        navigator.nextPage(
          UnknownPage,
          NormalMode,
          UserAnswers("id", SelectChange.ChangeActivity, contactAddress = contactAddress)
        ) mustBe defaultCall
      }
    }
    ...
    "Amount produced" - {
      ...
      def navigateFromAmountProduced(
        amountProduced: AmountProduced,
        mode: Mode,
        previousAmountProduced: AmountProduced
      ) =
        navigator.nextPage(
          AmountProducedPage,
          mode,
    ...
    "How many imports" - {
      "in CheckMode" - {
        def navigateFromHowManyImportsPageInCheckMode(userAnswers: UserAnswers) =
          navigator.nextPage(HowManyImportsPage, CheckMode, userAnswers)
    ...
    "Remove packaging site confirm with one packing site remaining" - {
      def navigateFromRemovePackagingSiteConfirm(value: Boolean, mode: Mode) =
        navigator.nextPage(
          ...
          emptyUserAnswersForChangeActivity
            .copy(packagingSiteList = packingSiteMap)
            .set(PackagingSiteDetailsPage, value)
            .success
            .value
        )
