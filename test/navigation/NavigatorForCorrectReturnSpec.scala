...
class NavigatorForCorrectReturnSpec extends SpecBase with DataHelper {
  ...
  val currentImporterSubscription: Option[RetrievedSubscription] = Option(
    aSubscription.copy(activity = testRetrievedActivity(importer = true))
  )
  val currentCoPackerSubscription: Option[RetrievedSubscription] = Option(
    aSubscription.copy(activity = testRetrievedActivity(contractPacker = true))
  )
    ...
    "in Normal mode" - {
      ...
      "must go from a page that doesn't exist in the route map to Index" in {
        ...
        navigator.nextPage(
          UnknownPage,
          NormalMode,
          UserAnswers("id", SelectChange.CorrectReturn, contactAddress = contactAddress)
        ) mustBe defaultCall
      }
  ...
  "Brought into UK" - {
    def navigateFromBroughtIntoUkPage(value: Boolean, mode: Mode) =
      navigator.nextPage(
        BroughtIntoUKPage,
        mode,
        emptyUserAnswersForCorrectReturn.set(BroughtIntoUKPage, value).success.value
      )
  ...
  "How many brought into UK" - {
    def navigateFromHowManyBroughtIntoUkPage(mode: Mode) =
      navigator.nextPage(
        ...
        mode,
        emptyUserAnswersForCorrectReturn.set(HowManyBroughtIntoUKPage, LitresInBands(1, 1)).success.value
      )
    ...
    "navigate to Check Your Answers page in CheckMode (when not new importer or new copacker)" in {
      val userAnswers = emptyUserAnswersForCorrectReturn
        .set(HowManyBroughtIntoUKPage, LitresInBands(1, 1))
        .success
        .value
      val result =
        navigator.nextPage(HowManyBroughtIntoUKPage, CheckMode, userAnswers, subscription = Some(aSubscription))
      result mustBe routes.CorrectReturnCYAController.onPageLoad
    }
    ...
    "navigate to Check Your Answers page in CheckMode (when already importer and already copacker)" in {
      ...
      val result = navigator.nextPage(
        ...
        CheckMode,
        userAnswers,
        subscription = currentImporterAndCoPackerSubscription
      )
      result mustBe routes.CorrectReturnCYAController.onPageLoad
    }
    ...
    "navigate to Check Your Answers page in CheckMode (when currently importer)" in {
      val userAnswers = newImporterUserAnswers
        ...
        .value
      val result =
        navigator.nextPage(HowManyBroughtIntoUKPage, CheckMode, userAnswers, None, currentImporterSubscription)
      result mustBe routes.CorrectReturnCYAController.onPageLoad
    }
    ...
    "navigate to Check Your Answers page in CheckMode (when currently copacker)" in {
      val userAnswers = newCoPackerUserAnswers
        ...
        .value
      val result =
        navigator.nextPage(HowManyBroughtIntoUKPage, CheckMode, userAnswers, None, currentCoPackerSubscription)
      result mustBe routes.CorrectReturnCYAController.onPageLoad
    }
    ...
    def navigateFromBroughtIntoUkFromSmallProducersPage(value: Boolean, mode: Mode) =
      navigator.nextPage(
        ...
        emptyUserAnswersForCorrectReturn
          .set(BroughtIntoUkFromSmallProducersPage, value)
          .success
          .value
      )
  ...
  "How many brought into UK from small producers" - {
    def navigateFromHowManyBroughtIntoUkFromSmallProducersPage(mode: Mode) =
      navigator.nextPage(
        ...
        mode,
        emptyUserAnswersForCorrectReturn
          .set(HowManyBroughtIntoUkFromSmallProducersPage, LitresInBands(1, 1))
          .success
          .value
      )
  ...
  "How many packaged as contract packer" - {
    def navigateFromHowManyPackagedAsContractPackerPage(mode: Mode) =
      navigator.nextPage(
        ...
        mode,
        emptyUserAnswersForCorrectReturn
          .set(HowManyPackagedAsContractPackerPage, LitresInBands(1, 1))
          .success
          .value
      )
    ...
    "navigate to Check Your Answers page in CheckMode (when currently importer)" in {
      ...
      val result = navigator.nextPage(
        ...
        CheckMode,
        userAnswers,
        subscription = currentImporterSubscription
      )
      result mustBe routes.CorrectReturnCYAController.onPageLoad
    }
  ...
  "Exemptions for small producers" - {
    ...
    def navigateFromExemptionsForSmallProducers(
      value: Boolean,
      mode: Mode,
      populateSmallProducerList: Boolean = false
    ) = {
      val userAnswers = emptyUserAnswersForCorrectReturn
        .copy(smallProducerList = if (populateSmallProducerList) smallProducerList else List.empty)
        .set(ExemptionsForSmallProducersPage, value)
        .success
        .value
    ...
    List(NormalMode, CheckMode).foreach { mode =>
      s"must go from $AskSecondaryWarehouseInReturnPage to $CorrectReturnBaseCYAPage in $mode when user selects no" in {
        navigator.nextPage(
          ...
          mode,
          UserAnswers("id", SelectChange.CorrectReturn, contactAddress = contactAddress)
            .set(AskSecondaryWarehouseInReturnPage, false)
            .success
            .value
        ) mustBe routes.CorrectReturnCYAController.onPageLoad
      }
    }
  }
  ...
  "Add A Small Producer " - {
    ...
    "Should navigate to small producer details controller when data is entered" in {
      navigator.nextPage(
        ...
        CheckMode,
        emptyUserAnswersForCorrectReturn.copy(data =
          Json.obj("addASmallProducer" -> Json.obj("lowBand" -> "10000", "highBand" -> "20000"))
        )
  ...
  s"must go from $AddASmallProducerPage to $SmallProducerDetailsPage in EditMode" in {
    navigator.nextPage(
      ...
      EditMode,
      UserAnswers("id", SelectChange.CorrectReturn, contactAddress = contactAddress)
    ) mustBe controllers.correctReturn.routes.SmallProducerDetailsController.onPageLoad(NormalMode)
  }
  ...
  List(NormalMode, CheckMode).foreach { mode =>
    s"must go from $AddASmallProducerPage to $SmallProducerDetailsPage in $mode" in {
      navigator.nextPage(
        ...
        mode,
        UserAnswers("id", SelectChange.CorrectReturn, contactAddress = contactAddress)
      ) mustBe controllers.correctReturn.routes.SmallProducerDetailsController.onPageLoad(mode)
    }
    ...
    s"must go from $RemoveSmallProducerConfirmPage to $ExemptionsForSmallProducersPage in $mode when zero small producers left" in {
      navigator.nextPage(
        ...
        UserAnswers("id", SelectChange.CorrectReturn, contactAddress = contactAddress)
          ...
          .success
          .value
      ) mustBe controllers.correctReturn.routes.ExemptionsForSmallProducersController.onPageLoad(mode)
    }
  ...
  "Claim Credits for Exports " - {
    def navigateFromClaimCreditsForExportsPage(value: Boolean, mode: Mode) =
      navigator.nextPage(
        ...
        mode,
        emptyUserAnswersForCorrectReturn.set(ClaimCreditsForExportsPage, value).success.value
      )
  ...
  "How many claim credits for exports " - {
    def navigateFromHowManyClaimCreditsForExportsPage(mode: Mode) =
      navigator.nextPage(
        HowManyClaimCreditsForExportsPage,
        mode,
        emptyUserAnswersForCorrectReturn
          .set(HowManyClaimCreditsForExportsPage, LitresInBands(1, 1))
          .success
          .value
      )
  ...
  "Claim Credits for Lost Damaged " - {
    def navigateFromClaimCreditsForLostDamagedPage(
      value: Boolean,
      mode: Mode,
      ...
      navigator.nextPage(
        ...
        mode,
        emptyUserAnswersForCorrectReturn.set(ClaimCreditsForLostDamagedPage, value).success.value,
        None,
        subscription
      )
    ...
    "select no to navigate to Check Your Answers page in NormalMode (when currently importer)" in {
      val userAnswers = newImporterUserAnswers
        ...
        .value
      val result =
        navigator.nextPage(ClaimCreditsForLostDamagedPage, NormalMode, userAnswers, None, currentImporterSubscription)
      result mustBe routes.CorrectReturnCYAController.onPageLoad
    }
    ...
    "select no to navigate to Check Your Answers page in NormalMode (when currently copacker)" in {
      val userAnswers = newCoPackerUserAnswers
        ...
        .value
      val result =
        navigator.nextPage(ClaimCreditsForLostDamagedPage, NormalMode, userAnswers, None, currentCoPackerSubscription)
      result mustBe routes.CorrectReturnCYAController.onPageLoad
    }
    ...
    "select no to navigate to Return Change Registration page in NormalMode (when new importer)" in {
      val userAnswers = newImporterUserAnswers
        ...
        .value
      val result =
        navigator.nextPage(ClaimCreditsForLostDamagedPage, NormalMode, userAnswers, None, Some(aSubscription))
  ...
  "How many claim credits for lost damaged " - {
    def navigateFromHowManyClaimCreditsForLostDamagedPage(
      mode: Mode,
      subscription: Option[RetrievedSubscription] = None
    ) =
      navigator.nextPage(
        ...
        mode,
        emptyUserAnswersForCorrectReturn.set(HowManyCreditsForLostDamagedPage, LitresInBands(1, 1)).success.value,
        None,
        subscription
      )
    ...
    "navigate to Check Your Answers page in CheckMode (when already importer and already copacker)" in {
      val userAnswers = emptyUserAnswersForCorrectReturn
        .set(HowManyCreditsForLostDamagedPage, LitresInBands(1, 1))
        .success
        .value
      val result = navigator.nextPage(
        HowManyCreditsForLostDamagedPage,
    ...
    "navigate to Check Your Answers page in NormalMode (when currently importer)" in {
      val userAnswers = newImporterUserAnswers
        ...
        .value
      val result =
        navigator.nextPage(HowManyCreditsForLostDamagedPage, NormalMode, userAnswers, None, currentImporterSubscription)
      result mustBe routes.CorrectReturnCYAController.onPageLoad
    }
    ...
    "navigate to Check Your Answers page in NormalMode (when currently copacker)" in {
      val userAnswers = newCoPackerUserAnswers
        ...
        .value
      val result =
        navigator.nextPage(HowManyCreditsForLostDamagedPage, NormalMode, userAnswers, None, currentCoPackerSubscription)
      result mustBe routes.CorrectReturnCYAController.onPageLoad
    }
    ...
    "navigate to Return Change Registration page in NormalMode (when new importer)" in {
      val userAnswers = newImporterUserAnswers
        ...
        .value
      val result =
        navigator.nextPage(HowManyCreditsForLostDamagedPage, NormalMode, userAnswers, None, Some(aSubscription))
      result mustBe routes.ReturnChangeRegistrationController.onPageLoad(NormalMode)
    }
  ...
  "Return change registration page " - {
    def navigateFromReturnChangeRegistrationPageWhenNewImporter(
      mode: Mode,
    ...
    def navigateFromReturnChangeRegistrationPageWhenNewCoPacker(
      mode: Mode,
      subscription: Option[RetrievedSubscription] = Some(aSubscription)
    ) =
      navigator.nextPage(
    ...
    def navigateFromReturnChangeRegistrationPageWhenNewImporterButUserHasSites(
      mode: Mode,
      subscription: Option[RetrievedSubscription] = Some(aSubscription)
    ) =
      navigator.nextPage(
    ...
    def navigateFromReturnChangeRegistrationPageWhenNewCoPackerButUserHasSites(
      mode: Mode,
      subscription: Option[RetrievedSubscription] = Some(aSubscription)
    ) =
      navigator.nextPage(
  ...
  "Packaging Site Details " - {
    def navigateFromPackagingSiteDetailsPage(mode: Mode, subscription: Option[RetrievedSubscription] = None) =
      navigator.nextPage(
        ...
        mode,
        emptyUserAnswersForCorrectReturn.set(PackagingSiteDetailsPage, false).success.value,
        None,
        subscription
      )
    ...
    "navigate to Check Your Answers page in NormalMode (when currently importer)" in {
      val userAnswers = newImporterUserAnswers
        ...
        .value
      val result =
        navigator.nextPage(PackagingSiteDetailsPage, NormalMode, userAnswers, None, currentImporterSubscription)
      result mustBe routes.CorrectReturnCYAController.onPageLoad
    }
    ...
    def navigateFromRemoveWarehouseConfirm(value: Boolean, mode: Mode, lastWarehouse: Boolean) =
      navigator.nextPage(
        ...
        emptyUserAnswersForCorrectReturn
          ...
          .set(RemoveWarehouseDetailsPage, value)
          .success
          .value
      )
