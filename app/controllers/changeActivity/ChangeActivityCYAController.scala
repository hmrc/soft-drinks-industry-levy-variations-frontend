...
class ChangeActivityCYAController @Inject() (
  ...
  private def submitUserAnswers(userAnswers: UserAnswers, subscription: RetrievedSubscription)(implicit
    ...
    changeActivityOrchestrator.submitVariation(subscription, userAnswers).value.flatMap {
      case Right(_) =>
        Future.successful(Redirect(controllers.changeActivity.routes.ChangeActivitySentController.onPageLoad()))
      case Left(_) =>
        genericLogger.logger
          .error(s"${getClass.getName} - ${userAnswers.id} - failed to submit change activity variation")
