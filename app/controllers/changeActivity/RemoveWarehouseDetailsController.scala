...
class RemoveWarehouseDetailsController @Inject() (
  ...
  def onPageLoad(index: String, mode: Mode): Action[AnyContent] =
    controllerActions.withRequiredJourneyData(ChangeActivity) { implicit request =>
      ...
      warehouseToRemove match {
        ...
        case Some(warehouse) =>
          ...
          form
            .bindFromRequest()
            .fold(
              formWithErrors => Future.successful(BadRequest(view(formWithErrors, formattedAddress, index, mode))),
              value => {
