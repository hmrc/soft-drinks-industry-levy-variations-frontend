...
class ReturnChangeRegistrationController @Inject() (
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = controllerActions.withCorrectReturnJourneyData { implicit request =>
    Redirect(
      navigator
        .nextPage(ReturnChangeRegistrationPage, mode, request.userAnswers, subscription = Some(request.subscription))
    )
  }
