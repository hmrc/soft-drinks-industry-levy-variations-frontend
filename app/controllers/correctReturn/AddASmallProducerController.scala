...
class AddASmallProducerController @Inject() (
      ...
      form
        ...
        .fold(
          ...
          value => {
            ...
            isValidSDILRef(None, value.referenceNumber, smallProducerList, request.returnPeriod).flatMap {
              case Left(AlreadyExists) =>
                Future.successful(
                  BadRequest(
                    view(
                      preparedForm.withError(
                        FormError("referenceNumber", "correctReturn.addASmallProducer.error.referenceNumber.exists")
                      ),
                      mode
                    )
                  )
                  ...
                  BadRequest(
                    view(
                      preparedForm.withError(
                        FormError(
                          "referenceNumber",
                          "correctReturn.addASmallProducer.error.referenceNumber.notASmallProducer"
                        )
                      ),
                      mode
                    )
              ...
              case Right(_) =>
                val updatedAnswers: UserAnswers = request.userAnswers
                  .copy(smallProducerList =
                    AddASmallProducer.toSmallProducer(value) :: request.userAnswers.smallProducerList
                  )
  ...
  def onEditPageLoad(mode: Mode, sdilReference: String): Action[AnyContent] =
    controllerActions.withCorrectReturnJourneyData.async { implicit request =>

      val form = formProvider(request.userAnswers)
      val targetSmallProducer =
        request.userAnswers.smallProducerList.find(producer => producer.sdilRef == sdilReference)
      ...
      targetSmallProducer match {
        case Some(producer) =>
          val addASmallProducer = AddASmallProducer(
            Some(producer.alias),
            producer.sdilRef,
            LitresInBands(producer.litreage.lower, producer.litreage.higher)
          )
      ...
      form
        ...
        .fold(
          ...
          value => {
            ...
            val preparedForm = form.fill(value)
            isValidSDILRef(Option(sdilReference), value.referenceNumber, smallProducerList, request.returnPeriod)
              .flatMap {
                case Left(AlreadyExists) =>
                  Future.successful(
                    BadRequest(
                ...
                case Left(_) =>
                  errorHandler.internalServerErrorTemplate.map(errorView => InternalServerError(errorView))
                case Right(_) =>
                  val newListWithOldSPRemoved =
                    request.userAnswers.smallProducerList.filterNot(_.sdilRef == sdilReference)
                  val updatedAnswers: UserAnswers = request.userAnswers
  ...
  private def isValidSDILRef(
    ...
    if (currentSDILRef.contains(addASmallProducerSDILRef)) {
