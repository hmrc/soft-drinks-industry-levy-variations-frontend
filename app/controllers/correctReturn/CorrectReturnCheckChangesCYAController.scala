...
class CorrectReturnCheckChangesCYAController @Inject() (
          ...
          updatedAnswers match {
            case Failure(_) =>
              genericLogger.logger.error(
                s"Failed to resolve user answers while on ${CorrectReturnCheckChangesPage.toString}"
              )
  ...
  def onSubmit: Action[AnyContent] = controllerActions.withCorrectReturnJourneyData.async { implicit request =>
    correctReturnOrchestrator
      .submitReturn(request.userAnswers, request.subscription, request.returnPeriod, request.originalSdilReturn)
      .value
      .flatMap {
        case Right(_) => Future.successful(Redirect(routes.CorrectReturnUpdateDoneController.onPageLoad().url))
        case Left(_) =>
          genericLogger.logger.error(
            s"${getClass.getName} - ${request.userAnswers.id} - received a failed response from return submission"
          )
