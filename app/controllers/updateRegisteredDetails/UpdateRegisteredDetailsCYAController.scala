  ...
  def onPageLoad: Action[AnyContent] = controllerActions.withRequiredJourneyData(UpdateRegisteredDetails) {
    implicit request =>
      val ukSiteDetailsSummary: Option[(String, SummaryList)] =
        UKSitesSummary.getHeadingAndSummary(request.userAnswers, true)
      val updateContactDetailsSummary: Option[(String, SummaryList)] =
        UpdateContactDetailsSummary.rows(request.userAnswers)
      ...
      updateRegisteredDetailsOrchestrator.submitVariation(subscription, userAnswers).value.flatMap {
        ...
        case Left(_) =>
          genericLogger.logger
            .error(s"${getClass.getName} - ${request.userAnswers.id} - failed to update registered details")
