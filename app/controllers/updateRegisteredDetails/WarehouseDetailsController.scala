...
class WarehouseDetailsController @Inject() (
    ...
    implicit request =>
      ...
      val summaryList: Option[SummaryList] = request.userAnswers.warehouseList match {
        case warehouseList if warehouseList.nonEmpty =>
          Some(SummaryListViewModel(rows = WarehouseDetailsSummary.row2(warehouseList, mode)))
        case _ => None
      }
  ...
  private def getOnwardUrl(value: Boolean, addContactDetails: Boolean, addBusinessAddress: Boolean, mode: Mode)(implicit
    hc: HeaderCarrier,
    ec: ExecutionContext,
    messages: Messages,
    ...
    if (value) {
      ...
      Future.successful((mode, addContactDetails, addBusinessAddress) match {
        ...
        case (_, true, _) =>
          controllers.updateRegisteredDetails.routes.UpdateContactDetailsController.onPageLoad(mode).url
