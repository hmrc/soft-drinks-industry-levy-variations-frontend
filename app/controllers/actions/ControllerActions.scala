...
class ControllerActions @Inject() (
  ...
  def withRequiredJourneyData[A](journeyType: SelectChange): ActionBuilder[DataRequest, AnyContent] =
    identify andThen getData andThen journeyDataRequiredAction(journeyType) andThen checkSubmission(
      onPostSubmissionPageLoad = false
    )

  def withRequiredJourneyDataPostSubmission[A](journeyType: SelectChange): ActionBuilder[DataRequest, AnyContent] =
    identify andThen getData andThen journeyDataRequiredAction(journeyType) andThen checkSubmission(
      onPostSubmissionPageLoad = true
    )

  def withCorrectReturnJourneyData[A]: ActionBuilder[CorrectReturnDataRequest, AnyContent] =
    identify andThen getData andThen correctReturnDataRequiredAction andThen checkReturnSubmission(
      onPostSubmissionPageLoad = false
    )
  ...
  private def correctReturnDataRequiredAction: ActionRefiner[OptionalDataRequest, CorrectReturnDataRequest] =
    new ActionRefiner[OptionalDataRequest, CorrectReturnDataRequest] {
      ...
      ): Future[Either[Result, CorrectReturnDataRequest[A]]] = {
        ...
        request.userAnswers match {
          case Some(userAnswers) if userAnswers.journeyType == SelectChange.CorrectReturn =>
            userAnswers.correctReturnPeriod match {
              ...
              case None =>
                Future.successful(Left(Redirect(controllers.correctReturn.routes.SelectController.onPageLoad)))
            }
      ...
      private def getOriginalReturnAndCreateDataRequest[T](
        ...
        sdilConnector.getReturn(request.subscription.utr, returnPeriod).value.flatMap {
          case Right(Some(sdilReturn)) =>
            Future.successful(
              Right(
                CorrectReturnDataRequest(
                  request.request,
                  request.sdilEnrolment,
      ...
      override protected def refine[A](request: OptionalDataRequest[A]): Future[Either[Result, DataRequest[A]]] =
        request.userAnswers match {
          ...
          case Some(userAnswers)
              if journeyType == SelectChange.CancelRegistration && userAnswers.journeyType == SelectChange.ChangeActivity =>
            Future.successful(
              Right(RequiredDataRequest(request.request, request.sdilEnrolment, request.subscription, userAnswers))
            )
          case None if request.subscription.deregDate.nonEmpty =>
            selectChangeOrchestrator
              .createCorrectReturnUserAnswersForDeregisteredUserAndSaveToDatabase(request.subscription)
              .value
              .flatMap {
                case Right(userAnswers) =>
                  Future.successful(
                    Right(
                      RequiredDataRequest(request.request, request.sdilEnrolment, request.subscription, userAnswers)
                    )
                  )
                case Left(_) =>
                  errorHandler
  ...
  private def preSubmissionResultFromJourneyType(selectChange: SelectChange): String = selectChange match {
    ...
    case UpdateRegisteredDetails =>
      controllers.updateRegisteredDetails.routes.ChangeRegisteredDetailsController.onPageLoad().url
  ...
  private def checkReturnSubmission(
    onPostSubmissionPageLoad: Boolean
  ): ActionRefiner[CorrectReturnDataRequest, CorrectReturnDataRequest] =
    new ActionRefiner[CorrectReturnDataRequest, CorrectReturnDataRequest] {
      override protected def refine[A](
        request: CorrectReturnDataRequest[A]
      ): Future[Either[Result, CorrectReturnDataRequest[A]]] =
  ...
  private def checkSubmission(onPostSubmissionPageLoad: Boolean): ActionRefiner[DataRequest, DataRequest] =
    new ActionRefiner[DataRequest, DataRequest] {
      override protected def refine[A](request: DataRequest[A]): Future[Either[Result, DataRequest[A]]] =
        (request.userAnswers.submitted, onPostSubmissionPageLoad) match {
          case (true, false) =>
            Future(Left(Redirect(postSubmissionResultFromJourneyType(request.userAnswers.journeyType))))
          case (false, true) =>
            Future(Left(Redirect(preSubmissionResultFromJourneyType(request.userAnswers.journeyType))))
          case _ => Future.successful(Right(request))
        }
    ...
    new ActionRefiner[OptionalDataRequest, DataRequest] {
      override protected def refine[T](request: OptionalDataRequest[T]): Future[Either[Result, DataRequest[T]]] =
        request.userAnswers match {
          case Some(data) =>
            Future.successful(
              Right(RequiredDataRequest(request.request, request.sdilEnrolment, request.subscription, data))
            )
          case _ => Future.successful(Left(Redirect(routes.SelectChangeController.onPageLoad)))
        }
