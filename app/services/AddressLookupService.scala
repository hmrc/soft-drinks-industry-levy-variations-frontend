...
class AddressLookupService @Inject() (
    ...
    addressLookupState match {
      case PackingDetails =>
        userAnswers.copy(packagingSiteList =
          userAnswers.packagingSiteList.filterNot(_._1 == sdilId) ++ Map(
            sdilId -> Site(convertedAddress, None, address.organisation, None)
          )
        )
      case WarehouseDetails =>
        userAnswers.copy(warehouseList =
          userAnswers.warehouseList.filterNot(_._1 == sdilId) ++ Map(
            sdilId -> Site(convertedAddress, address.organisation)
          )
        )
      case ContactDetails =>
              ...
              LanguageLabels(
                ...
                lookupPageLabels = Some(
                  LookupPageLabels(
                    ...
                    postcodeLabel =
                      Some(messages("addressLookupFrontend.contactDetails.lookupPageLabels.postcodeLabel"))
                  )
                ),
                editPageLabels = Some(
                  EditPageLabels(
  ...
  private def returnContinueUrl(state: AddressLookupState, sdilId: String, mode: Mode): String =
    state match {
      ...
      case ContactDetails   => frontendAppConfig.AddressLookupConfig.ContactDetails.offRampUrl(sdilId, mode)
    }
}
