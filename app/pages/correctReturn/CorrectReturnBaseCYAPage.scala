...
case object CorrectReturnBaseCYAPage extends QuestionPage[Boolean] {
  ...
  override val previousPagesRequired: (UserAnswers, RetrievedSubscription) => List[RequiredPage] =
    (userAnswers, subscription) =>
      List(
        RequiredPage(
          OperatePackagingSiteOwnBrandsPage,
          additionalPreconditions = List(!subscription.activity.smallProducer)
        ),
        RequiredPage(
        ...
        RequiredPage(
          ...
          additionalPreconditions = List(userAnswers.get(PackagedAsContractPackerPage).contains(true))
        ),
        ...
        RequiredPage(
          ...
          additionalPreconditions = List(userAnswers.get(BroughtIntoUKPage).contains(true))
        ),
        RequiredPage(BroughtIntoUkFromSmallProducersPage),
        RequiredPage(
          HowManyBroughtIntoUkFromSmallProducersPage,
          additionalPreconditions = List(userAnswers.get(BroughtIntoUkFromSmallProducersPage).contains(true))
        ),
        RequiredPage(ClaimCreditsForExportsPage),
        RequiredPage(
          HowManyClaimCreditsForExportsPage,
          additionalPreconditions = List(userAnswers.get(ClaimCreditsForExportsPage).contains(true))
        ),
        RequiredPage(ClaimCreditsForLostDamagedPage),
        RequiredPage(
          HowManyCreditsForLostDamagedPage,
          additionalPreconditions = List(userAnswers.get(ClaimCreditsForLostDamagedPage).contains(true))
        ),
        RequiredPage(
        ...
        RequiredPage(
          ...
          additionalPreconditions = List(UserTypeCheck.isNewImporter(userAnswers, subscription))
        )
      )
}
