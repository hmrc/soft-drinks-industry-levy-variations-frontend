    ...
    answers.get(AskSecondaryWarehouseInReturnPage).map {
      answer =>
        ...
        SummaryListRowViewModel(
          key     = "correctReturn.askSecondaryWarehouseInReturn.checkYourAnswersLabel",
          value   = ValueViewModel(value),
          actions = Seq(
            ActionItemViewModel("site.change", routes.AskSecondaryWarehouseInReturnController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("correctReturn.askSecondaryWarehouseInReturn.change.hidden"))
          )
        )
    }
}
