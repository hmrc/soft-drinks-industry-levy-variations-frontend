...
object PackAtBusinessAddressSummary  {
  //  TODO: ADD UNIT TESTS (IS THIS NEEDED)
    ...
    answers.get(PackAtBusinessAddressPage).map {
      answer =>
        ...
        SummaryListRowViewModel(
          key     = "changeActivity.packAtBusinessAddress.checkYourAnswersLabel",
          value   = ValueViewModel(value),
          actions = Seq(
            ActionItemViewModel("site.change", routes.PackAtBusinessAddressController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText(messages("changeActivity.packAtBusinessAddress.change.hidden"))
          )
        )
    }
}
