...
class NavigatorForCorrectReturn @Inject() () extends Navigator {
  ...
  private def navigationForExemptionsForSmallProducers(userAnswers: UserAnswers, mode: Mode): Call =
    if (userAnswers.get(page = ExemptionsForSmallProducersPage).contains(true)) {
      (mode, userAnswers.smallProducerList.nonEmpty, userAnswers.smallProducerList.isEmpty) match {
        case (CheckMode, true, _) => routes.SmallProducerDetailsController.onPageLoad(mode)
        case (_, true, false)     => routes.SmallProducerDetailsController.onPageLoad(mode)
  ...
  private def navigationForCreditsForLostDamagedInNormalMode(
  ...
  private def navigationForPackagingSiteDetailsInNormalMode(
    userAnswers: UserAnswers,
    subscription: RetrievedSubscription
  ) =
  ...
  private def navigationToReturnChangeRegistrationIfRequired(
    userAnswers: UserAnswers,
    subscription: RetrievedSubscription,
    mode: Mode
  ) = {
  ...
  private def navigationFromReturnChangeRegistration(
    userAnswers: UserAnswers,
    subscription: RetrievedSubscription,
    mode: Mode
  ): Call = {
  ...
  override val normalRoutes: Page => UserAnswers => Call = {
    case OperatePackagingSiteOwnBrandsPage =>
      userAnswers => navigationForOperatePackagingSiteOwnBrands(userAnswers, NormalMode)
    case HowManyOperatePackagingSiteOwnBrandsPage =>
      _ => routes.PackagedAsContractPackerController.onPageLoad(NormalMode)
    ...
    case ExemptionsForSmallProducersPage =>
      userAnswers => navigationForExemptionsForSmallProducers(userAnswers, NormalMode)
    ...
    case RemoveSmallProducerConfirmPage =>
      userAnswers => navigationForRemoveSmallProducerConfirm(userAnswers, NormalMode)
    ...
    case BroughtIntoUkFromSmallProducersPage =>
      userAnswers => navigationForBroughtIntoUkFromSmallProducers(userAnswers, NormalMode)
    case HowManyBroughtIntoUkFromSmallProducersPage =>
      _ => routes.ClaimCreditsForExportsController.onPageLoad(NormalMode)
    ...
    case RemovePackagingSiteConfirmPage =>
      userAnswers => navigationForRemovePackagingSiteConfirm(userAnswers, NormalMode)
  ...
  override val normalRoutesWithSubscription: Page => (UserAnswers, RetrievedSubscription) => Call = {
    case ClaimCreditsForLostDamagedPage =>
      (userAnswers, subscription) => navigationForCreditsForLostDamagedInNormalMode(userAnswers, subscription)
    case HowManyCreditsForLostDamagedPage =>
      (userAnswers, subscription) =>
        navigationToReturnChangeRegistrationIfRequired(userAnswers, subscription, NormalMode)
    case PackagingSiteDetailsPage =>
      (userAnswers, subscription) => navigationForPackagingSiteDetailsInNormalMode(userAnswers, subscription)
    case ReturnChangeRegistrationPage =>
      (userAnswers, subscription) => navigationFromReturnChangeRegistration(userAnswers, subscription, NormalMode)
    case _ => (_, _) => defaultCall
  }

  override val checkRouteMap: Page => UserAnswers => Call = {
    case OperatePackagingSiteOwnBrandsPage =>
      userAnswers => navigationForOperatePackagingSiteOwnBrands(userAnswers, CheckMode)
    ...
    case ExemptionsForSmallProducersPage =>
      userAnswers => navigationForExemptionsForSmallProducers(userAnswers, CheckMode)
    ...
    case RemoveSmallProducerConfirmPage =>
      userAnswers => navigationForRemoveSmallProducerConfirm(userAnswers, CheckMode)
    case BroughtIntoUKPage => userAnswers => navigationForBroughtIntoUK(userAnswers, CheckMode)
    case BroughtIntoUkFromSmallProducersPage =>
      userAnswers => navigationForBroughtIntoUkFromSmallProducers(userAnswers, CheckMode)
    ...
    case RemovePackagingSiteConfirmPage =>
      userAnswers => navigationForRemovePackagingSiteConfirm(userAnswers, CheckMode)
    case AskSecondaryWarehouseInReturnPage => _ => routes.CorrectReturnCYAController.onPageLoad
    case SecondaryWarehouseDetailsPage     => _ => routes.CorrectReturnCYAController.onPageLoad
  ...
  override val checkRouteMapWithSubscription: Page => (UserAnswers, RetrievedSubscription) => Call = {
    case HowManyBroughtIntoUKPage =>
      (userAnswers, subscription) =>
        navigationToReturnChangeRegistrationIfRequired(userAnswers, subscription, CheckMode)
    case HowManyPackagedAsContractPackerPage =>
      (userAnswers, subscription) =>
        navigationToReturnChangeRegistrationIfRequired(userAnswers, subscription, CheckMode)
    case ReturnChangeRegistrationPage =>
      (userAnswers, subscription) => navigationFromReturnChangeRegistration(userAnswers, subscription, CheckMode)
    case _ => (_, _) => defaultCall
  }
