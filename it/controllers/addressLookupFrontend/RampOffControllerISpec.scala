        ...
        "should redirect to next page when request is valid and address is returned from ALF when" - {
          "no address exists in DB currently for SDILID provided" in {
            ...
            WsTestClient.withClient { client =>
              val result = createClientRequestGet(
                client,
                s"$baseUrl/off-ramp/${if (mode == CheckMode) "change-" else ""}secondary-warehouses/$sdilId?id=$alfId"
              )
          ...
          "an address already exists in DB currently for SDILID provided" in {
            ...
            val userAnswersBefore =
              userAnswers.copy(warehouseList = Map(sdilId -> Site(UkAddress(List.empty, "foo", Some("wizz")))))
            build.commonPrecondition.alf.getAddress(alfId)
        ...
        "redirect to next page when request is valid and address is returned from ALF when" - {
          "no address exists in DB currently for SDILID provided" in {
            ...
            WsTestClient.withClient { client =>
              val result = createClientRequestGet(
                client,
                s"$baseUrl/off-ramp/${if (mode == CheckMode) "change-" else ""}packing-site-details/$sdilId?id=$alfId"
              )
      ...
      "should redirect to next page when request is valid and address is returned from ALF when" - {
        "no address exists in DB currently for SDILID provided" in {
          ...
          WsTestClient.withClient { client =>
            val result = createClientRequestGet(
              client,
              s"$baseUrl/off-ramp/${if (mode == CheckMode) "change-" else ""}business-address/$sdilId?id=$alfId"
            )
            ...
            whenReady(result) { res =>
              ...
              updatedUserAnswersNoContactDetails.contactAddress mustBe UkAddress(
        ...
        "an address already exists in DB currently for SDILID provided" in {
          ...
          val alfId: String = "bar"
          val userAnswersBefore =
            emptyUserAnswersForUpdateRegisteredDetails.copy(contactAddress = UkAddress(List.empty, "foo", Some("wizz")))
          build.commonPrecondition.alf.getAddress(alfId)
          setAnswers(userAnswersBefore)
          ...
          WsTestClient.withClient { client =>
            ...
            whenReady(result) { res =>
              ...
              updatedUserAnswers.contactAddress mustBe UkAddress(
                List("line 1", "line 2", "line 3", "line 4"),
                "aa1 1aa",
                alfId = Some(alfId)
              )
