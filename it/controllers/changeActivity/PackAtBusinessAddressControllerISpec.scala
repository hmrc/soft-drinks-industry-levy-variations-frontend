          ...
          "when the session contains no data for page" in {
            ...
            WsTestClient.withClient { client =>
              ...
              whenReady(result) { res =>
                ...
                updatedAnswers.map(data => data.packagingSiteList) mustBe Some(
                  if (yesSelected) packAtBusinessAddressSite else Map()
                )
              }
            }
          }
    ...
    "user selected no, user should be taken to ALF in normal mode" in {
      val journeyConfigToBePosted: JourneyConfig = JourneyConfig(
        ...
        options = JourneyOptions(
          continueUrl =
            s"http://localhost:8705/soft-drinks-industry-levy-variations-frontend/off-ramp/pack-at-business-address/$sdilNumber",
          homeNavHref = None,
          ...
          phaseFeedbackLink = Some(
            s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-variations-frontend&backUrl=http%3A%2F%2Flocalhost%3A8705%2Fsoft-drinks-industry-levy-variations-frontend%2Fchange-activity%2Fpack-at-business-address"
          ),
      ...
      WsTestClient.withClient { client =>
        val result1 =
          createClientRequestPOST(client, changeActivityBaseUrl + normalRoutePath, Json.obj("value" -> "false"))
