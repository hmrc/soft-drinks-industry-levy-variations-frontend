...
class SecondaryWarehouseDetailsControllerISpec extends ControllerITTestHelper {
  ...
  "GET " + normalRoutePath - {
    ...
    "when the userAnswers contains some warehouses" - {
      ...
      val multipleWarehouses =
        singleWarehouse ++ Map("2" -> Site(UkAddress(List("1 Watch Street"), "DF4 3WE"), Some("ACME Soft Drinks")))
            ...
            WsTestClient.withClient { client =>
              ...
              whenReady(result1) { res =>
                ...
                summaryList.text mustBe warehouseList.values
                  .map { warehouse =>
                    val removeText =
                      s"Remove warehouse ${warehouse.tradingName.getOrElse("")} at ${warehouse.address.lines.head}"
          ...
          WsTestClient.withClient { client =>
            ...
            whenReady(result) { res =>
              ...
              val dataStoredForPage =
                getAnswers(sdilNumber).fold[Option[Boolean]](None)(_.get(SecondaryWarehouseDetailsPage))
      ...
      val journeyConfigToBePosted: JourneyConfig = JourneyConfig(
        ...
        options = JourneyOptions(
          ...
          phaseFeedbackLink = Some(
            s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-variations-frontend&backUrl=http%3A%2F%2Flocalhost%3A8705%2Fsoft-drinks-industry-levy-variations-frontend%2Fchange-activity%2Fsecondary-warehouse-details"
          ),
      ...
      val journeyConfigToBePosted: JourneyConfig = JourneyConfig(
        ...
        options = JourneyOptions(
          ...
          accessibilityFooterUrl =
            Some("localhost/accessibility-statement/soft-drinks-industry-levy-variations-frontend"),
          phaseFeedbackLink = Some(
            s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-variations-frontend&backUrl=http%3A%2F%2Flocalhost%3A8705%2Fsoft-drinks-industry-levy-variations-frontend%2Fchange-activity%2Fchange-secondary-warehouse-details"
          ),
          deskProServiceName = None,
          showPhaseBanner = Some(false),
          alphaPhase = Some(false),
