...
class RemoveWarehouseDetailsControllerISpec extends ControllerITTestHelper {
  ...
  "GET " + normalRoutePath("indexDoesntExist") - {
    ...
    userAnswersForChangeActivityRemoveWarehouseDetailsPage(indexOfWarehouseToBeRemoved).foreach {
      case (key, userAnswers) =>
        s"when the userAnswers contains data for the page with " + key + " selected" - {
          s"should return OK and render the page without the " + key + " radio checked" in {
            ...
            WsTestClient.withClient { client =>
              val result =
                createClientRequestGet(client, changeActivityBaseUrl + normalRoutePath(indexOfWarehouseToBeRemoved))
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(
      ChangeActivity,
      changeActivityBaseUrl + normalRoutePath(indexOfWarehouseToBeRemoved)
    )
  }
  ...
  "GET " + checkRoutePath("indexDoesntExist") - {
      ...
      case (key, userAnswers) =>
        s"when the userAnswers contains data for the page with " + key + " selected" - {
          s"should return OK and render the page without the " + key + " radio checked" in {
            ...
            WsTestClient.withClient { client =>
              val result =
                createClientRequestGet(client, changeActivityBaseUrl + checkRoutePath(indexOfWarehouseToBeRemoved))
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(
      ChangeActivity,
      changeActivityBaseUrl + checkRoutePath(indexOfWarehouseToBeRemoved)
    )
  }
