        ...
        WsTestClient.withClient { client =>
          ...
          whenReady(result1) { res =>
            ...
            res.header(HeaderNames.LOCATION) mustBe Some(
              controllers.changeActivity.routes.PackAtBusinessAddressController.onPageLoad(NormalMode).url
            )
          }
        }
      }
      ...
      s"should redirect to PackAtBusinessAddress" in {
        ...
        WsTestClient.withClient { client =>
          ...
          whenReady(result1) { res =>
            res.status mustBe 303
            res.header(HeaderNames.LOCATION) mustBe Some(
              controllers.changeActivity.routes.PackAtBusinessAddressController.onPageLoad(CheckMode).url
            )
          }
        }
      }
          ...
          WsTestClient.withClient { client =>
            ...
            whenReady(result) { res =>
              ...
              val dataStoredForPage =
                getAnswers(updatedUserAnswersImports.id).fold[Option[Boolean]](None)(_.get(PackagingSiteDetailsPage))
    ...
    "user selected yes, user should be taken to ALF" in {
      val journeyConfigToBePosted: JourneyConfig = JourneyConfig(
        ...
        options = JourneyOptions(
          continueUrl =
            s"http://localhost:8705/soft-drinks-industry-levy-variations-frontend/off-ramp/packing-site-details/$sdilNumber",
          homeNavHref = None,
          ...
          phaseFeedbackLink = Some(
            s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-variations-frontend&backUrl=http%3A%2F%2Flocalhost%3A8705%2Fsoft-drinks-industry-levy-variations-frontend%2Fchange-activity%2Fpackaging-site-details"
          ),
        ...
        "when the session contains no data for page" in {
          ...
          WsTestClient.withClient { client =>
            ...
            whenReady(result) { res =>
              res.status mustBe 303
              res.header(HeaderNames.LOCATION) mustBe Some(
                controllers.changeActivity.routes.ChangeActivityCYAController.onPageLoad().url
              )
              val dataStoredForPage =
                getAnswers(updatedUserAnswersImports.id).fold[Option[Boolean]](None)(_.get(PackagingSiteDetailsPage))
              dataStoredForPage mustBe Some(false)
      ...
      val journeyConfigToBePosted: JourneyConfig = JourneyConfig(
        ...
        options = JourneyOptions(
          ...
          accessibilityFooterUrl =
            Some("localhost/accessibility-statement/soft-drinks-industry-levy-variations-frontend"),
          phaseFeedbackLink = Some(
            s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-variations-frontend&backUrl=http%3A%2F%2Flocalhost%3A8705%2Fsoft-drinks-industry-levy-variations-frontend%2Fchange-activity%2Fchange-packaging-site-details"
          ),
          deskProServiceName = None,
          showPhaseBanner = Some(false),
          alphaPhase = Some(false),
