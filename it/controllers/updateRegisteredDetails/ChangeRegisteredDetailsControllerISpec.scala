    ...
    ChangeRegisteredDetails.values.zipWithIndex.foreach { case (checkboxItem, index) =>
      s"when the userAnswers contains data for the page with " + checkboxItem.toString + " selected" - {
        s"should return OK and render the page with " + checkboxItem.toString + " checkboxItem checked" in {
          ...
          val userAnswers =
            emptyUserAnswersForUpdateRegisteredDetails.set(ChangeRegisteredDetailsPage, Seq(checkboxItem)).success.value
  ...
  s"POST " + normalRoutePath - {
      ...
      "when the user selects " + checkboxItem.toString - {
        "should update the session with the new value and redirect to the index controller" - {
          "when the session contains no data for page" in {
            ...
            WsTestClient.withClient { client =>
              val result = createClientRequestPOST(
                client,
                updateRegisteredDetailsBaseUrl + normalRoutePath,
                Json.obj("value" -> Set(checkboxItem))
              )
              ...
              whenReady(result) { res =>
                ...
                checkboxItem match {
                  case ChangeRegisteredDetails.Sites =>
                    res.header(HeaderNames.LOCATION) mustBe Some(
                      routes.PackagingSiteDetailsController.onPageLoad(NormalMode).url
                    )
                  case ChangeRegisteredDetails.ContactDetails =>
                    res.header(HeaderNames.LOCATION) mustBe Some(
                      routes.UpdateContactDetailsController.onPageLoad(NormalMode).url
                    )
                  case ChangeRegisteredDetails.BusinessAddress =>
                    res.header(HeaderNames.LOCATION) mustBe Some(routes.BusinessAddressController.onPageLoad().url)
                }

                val dataStoredForPage = getAnswers(sdilNumber).fold[Option[Seq[ChangeRegisteredDetails]]](None)(
                  _.get(ChangeRegisteredDetailsPage)
                )
          ...
          "when the session already contains data for page" in {
            ...
            val userAnswers = emptyUserAnswersForUpdateRegisteredDetails
              .set(ChangeRegisteredDetailsPage, Seq(checkboxItem))
              .success
              .value
        ...
        "when the session contains no data for page" in {
          ...
          WsTestClient.withClient { client =>
            val result = createClientRequestPOST(
              client,
              updateRegisteredDetailsBaseUrl + normalRoutePath,
              Json.obj("value" -> ChangeRegisteredDetails.values)
            )
    ...
    "when the user activity is voluntary true" - {
      "when value 'sites' is attempted to be saved, should return 400 with required error" in {
        ...
        val userAnswers = emptyUserAnswersForUpdateRegisteredDetails
          .set(ChangeRegisteredDetailsPage, ChangeRegisteredDetails.voluntaryValues)
          .success
          .value
