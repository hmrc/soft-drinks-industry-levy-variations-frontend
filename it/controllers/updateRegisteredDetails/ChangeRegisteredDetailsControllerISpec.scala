...
import org.scalatest.matchers.must.Matchers.{convertToAnyMustWrapper, have, not}
import org.scalatest.matchers.must.Matchers.not.be
...
class ChangeRegisteredDetailsControllerISpec extends ControllerITTestHelper {
  ...
  s"POST " + normalRoutePath - {
    ChangeRegisteredDetails.values.foreach { case checkboxItem =>
    ...
    "when the user activity is voluntary true" - {
      "when value 'sites' is attempted to be saved, should return 400 with required error" in {
        ...
        val userAnswers = emptyUserAnswersForUpdateRegisteredDetails.set(ChangeRegisteredDetailsPage, ChangeRegisteredDetails.voluntaryValues.toSet).success.value
