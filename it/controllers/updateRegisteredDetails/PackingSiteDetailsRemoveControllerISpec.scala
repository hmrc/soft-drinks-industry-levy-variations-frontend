...
class PackingSiteDetailsRemoveControllerISpec extends ControllerITTestHelper {
  ...
  "GET UpdateRegisteredDetails - " + normalRoutePath("indexDoesntExist") - {
    "when the userAnswers contains no data" - {
      "should redirect away when no data exists" in {
        ...
        WsTestClient.withClient { client =>
          val result =
            createClientRequestGet(client, updateRegisteredDetailsBaseUrl + normalRoutePath("indexDoesntExist"))
    ...
    userAnswersForUpdateRegisteredDetailsPackingSiteDetailsRemovePage(indexOfPackingSiteToBeRemoved).foreach {
          ...
          s"should return OK and render the page without the " + key + " radio checked" in {
            ...
            WsTestClient.withClient { client =>
              val result = createClientRequestGet(
                client,
                updateRegisteredDetailsBaseUrl + normalRoutePath(indexOfPackingSiteToBeRemoved)
              )
  ...
  s"GET " + checkRoutePath(indexOfPackingSiteToBeRemoved) - {
    "when the userAnswers contains no data" - {
      "should redirect away when no data exists" in {
        ...
        WsTestClient.withClient { client =>
          val result =
            createClientRequestGet(client, updateRegisteredDetailsBaseUrl + checkRoutePath("indexDoesntExist"))
        ...
        s"when the userAnswers contains data for the page with " + key + " selected" - {
          s"should return OK and render the page without the " + key + " radio checked" in {
            ...
            WsTestClient.withClient { client =>
              val result = createClientRequestGet(
                client,
                updateRegisteredDetailsBaseUrl + checkRoutePath(indexOfPackingSiteToBeRemoved)
              )
              ...
              WsTestClient.withClient { client =>
                ...
                whenReady(result) { res =>
                  ...
                  val dataStoredForPage =
                    userAnswersAfterTest.fold[Option[Boolean]](None)(_.get(PackingSiteDetailsRemovePage))
                  if (yesSelected) {
