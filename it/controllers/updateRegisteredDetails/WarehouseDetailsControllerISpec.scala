...
class WarehouseDetailsControllerISpec extends ControllerITTestHelper {
  ...
  val userAnswersWithAllChangeRegisteredDetailsSelections: UserAnswers = emptyUserAnswersForUpdateRegisteredDetails
    .set(ChangeRegisteredDetailsPage, selectedChangeRegisteredDetails)
    .success
    .value
      ...
      "when the userAnswers contains some warehouses" - {
        "should return OK and render the WarehouseDetails page with no data populated " +
          ...
          "with subheading asking if user would like to add another warehouse" in {
            ...
            setAnswers(
              emptyUserAnswersForUpdateRegisteredDetails.copy(warehouseList =
  ...
  s"GET " + checkRoutePath - {
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(
      UpdateRegisteredDetails,
      updateRegisteredDetailsBaseUrl + checkRoutePath
    )
  }

  val userAnswersWithSiteOnlyChangeRegisteredDetailsSelections: UserAnswers = emptyUserAnswersForUpdateRegisteredDetails
    .set(ChangeRegisteredDetailsPage, Seq(Sites))
    .success
    .value
  val userAnswersWithSitesAndContactDetailsChangeRegisteredDetailsSelections: UserAnswers =
    emptyUserAnswersForUpdateRegisteredDetails
      .set(ChangeRegisteredDetailsPage, Seq(Sites, ContactDetails))
      .success
      .value
  val userAnswersWithSitesAndBusinessAddressChangeRegisteredDetailsSelections: UserAnswers =
    emptyUserAnswersForUpdateRegisteredDetails
      .set(ChangeRegisteredDetailsPage, Seq(Sites, BusinessAddress))
      .success
      .value
  ...
  s"POST " + normalRoutePath - {
              ...
              WsTestClient.withClient { client =>
                ...
                whenReady(result) { res =>
                  ...
                  res.header(HeaderNames.LOCATION) mustBe
                    Some(
                      controllers.updateRegisteredDetails.routes.UpdateContactDetailsController
        ...
        val journeyConfigToBePosted: JourneyConfig = JourneyConfig(
          ...
          options = JourneyOptions(
            ...
            phaseFeedbackLink = Some(
              s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-variations-frontend&backUrl=http%3A%2F%2Flocalhost%3A8705%2Fsoft-drinks-industry-levy-variations-frontend%2Fchange-registered-details%2Fwarehouse-details"
            ),
    ...
    "when the user does not select yes or no" - {
      ...
      "when the user has warehouses and does not select yes or no" - {
        "should return 400 with required error" in {
          build.commonPrecondition
          val userAnswersWithWarehouses =
            emptyUserAnswersForUpdateRegisteredDetails.copy(warehouseList = warehousesFromSubscription)
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(
      UpdateRegisteredDetails,
      updateRegisteredDetailsBaseUrl +
        normalRoutePath,
      Some(Json.obj("value" -> "true"))
    )
  }
        ...
        "when the session contains no data for page" in {
          ...
          WsTestClient.withClient { client =>
            ...
            whenReady(result) { res =>
              res.status mustBe 303
              res.header(HeaderNames.LOCATION) mustBe Some(
                controllers.updateRegisteredDetails.routes.UpdateContactDetailsController.onPageLoad(NormalMode).url
              )
              val dataStoredForPage =
                getAnswers(userAnswers.id).fold[Option[Boolean]](None)(_.get(WarehouseDetailsPage))
              dataStoredForPage.isEmpty mustBe true
        ...
        "when the session contains no data for page" in {
          ...
          WsTestClient.withClient { client =>
            ...
            whenReady(result) { res =>
              res.status mustBe 303
              res.header(HeaderNames.LOCATION) mustBe Some(
                controllers.updateRegisteredDetails.routes.BusinessAddressController.onPageLoad().url
              )
              val dataStoredForPage =
                getAnswers(userAnswers.id).fold[Option[Boolean]](None)(_.get(WarehouseDetailsPage))
              dataStoredForPage.isEmpty mustBe true
