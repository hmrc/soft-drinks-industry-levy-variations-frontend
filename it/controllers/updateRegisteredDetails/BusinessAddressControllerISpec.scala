...
class BusinessAddressControllerISpec extends ControllerITTestHelper {
  ...
  "GET " + changeBusinessAddressPath - {
    "should redirect to ALF" in {
      val journeyConfigToBePosted: JourneyConfig = JourneyConfig(
        ...
        options = JourneyOptions(
          continueUrl =
            s"http://localhost:8705/soft-drinks-industry-levy-variations-frontend/off-ramp/business-address/$sdilNumber",
          homeNavHref = None,
          ...
          phaseFeedbackLink = Some(
            s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-variations-frontend&backUrl=http%3A%2F%2Flocalhost%3A8705%2Fsoft-drinks-industry-levy-variations-frontend%2Fchange-registered-details%2Fchange-business-address"
          ),
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(
      ...
      updateRegisteredDetailsBaseUrl + changeBusinessAddressPath
    )
  }
  "GET " + normalRoutePath - {
  ...
  "POST " + normalRoutePath - {
    "should return OK and render the BusinessAddress page" in {
      ...
      WsTestClient.withClient { client =>
        val result1 =
          createClientRequestPOST(client, updateRegisteredDetailsBaseUrl + normalRoutePath, Json.obj("value" -> "true"))
