...
class CancelRegistrationDateControllerISpec extends ControllerITTestHelper {
  ...
  val validCancellationDateJson = Json.obj(
    "value.day" -> validCancellationDate.getDayOfMonth.toString,
    "value.month" -> validCancellationDate.getMonth.getValue.toString,
    "value.year" -> validCancellationDate.getYear.toString
  )
        ...
        "when only the " + field + "is populated" in {
          ...
          WsTestClient.withClient { client =>
            ...
            whenReady(result) { res =>
              ...
              errorSummary.text() mustBe Messages("cancelRegistration.cancelRegistrationDate" + ".error.required.two", otherFields(0), otherFields(1)
              )
            }
          }
        }
          ...
          WsTestClient.withClient { client =>
            ...
            whenReady(result) { res =>
              ...
              errorSummary
                .select("a")
                .attr("href") mustBe "#value.day"
              errorSummary.text() mustBe Messages("cancelRegistration.cancelRegistrationDate" + ".error.required", field
              )
            }
          }
        }
        ...
        WsTestClient.withClient { client =>
          ...
          whenReady(result) { res =>
            ...
            errorSummary
              .select("a")
              .attr("href") mustBe "#value.day"
            errorSummary.text() mustBe Messages("cancelRegistration.cancelRegistrationDate" + ".error.required.all"
            )
          }
        }
      }
        ...
        WsTestClient.withClient { client =>
          ...
          whenReady(result) { res =>
            ...
            errorSummary
              .select("a")
              .attr("href") mustBe "#value.day"
            errorSummary.text() mustBe Messages("cancelRegistration.cancelRegistrationDate" + ".error.invalid"
            )
          }
        }
      }
    }
    testUnauthorisedUser(cancelRegistrationBaseUrl + normalRoutePath, Some(Json.obj("value" -> "true")))
    testAuthenticatedUserButNoUserAnswers(cancelRegistrationBaseUrl + normalRoutePath, Some(Json.obj("value" -> "true")))
  }
