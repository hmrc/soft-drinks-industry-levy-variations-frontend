...
class CorrectionReasonControllerISpec extends ControllerITTestHelper {
  ...
  val randomStringExceedingMaxLength = Random.nextString(255 + 1)

  val userAnswers = emptyUserAnswersForCorrectReturn.set(CorrectionReasonPage, correctionReason).success.value
      ...
      "should return OK and render the CorrectionReason page with no data populated" in {
        ...
        WsTestClient.withClient { client =>
          ...
          whenReady(result1) { res =>
            ...
            page.title must include(Messages("correctReturn.correctionReason" + ".title"))
  ...
  s"POST " + normalRoutePath - {
      ...
      "when the question is not answered" in {
        ...
        WsTestClient.withClient { client =>
          ...
          whenReady(result) { res =>
            ...
            val page = Jsoup.parse(res.body)
            page.title must include("Error: " + Messages("correctReturn.correctionReason" + ".title"
            ) )
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(CorrectReturn, correctReturnBaseUrl + normalRoutePath, Some(Json.obj("value" -> correctionReasonDiff)))
  }
  ...
  s"POST " + checkRoutePath - {
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(CorrectReturn, correctReturnBaseUrl + checkRoutePath, Some(Json.obj("value" -> correctionReasonDiff)))
  }
}
