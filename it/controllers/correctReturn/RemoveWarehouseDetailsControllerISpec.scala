...
class RemoveWarehouseDetailsControllerISpec extends ControllerITTestHelper {
  ...
  "GET " + normalRoutePath("indexDoesntExist") - {
    ...
    userAnswersForCorrectReturnRemoveWarehouseDetailsPage(indexOfWarehouseToBeRemoved).foreach {
      case (key, userAnswers) =>
        s"when the userAnswers contains data for the page with " + key + " selected" - {
          s"should return OK and render the page without the " + key + " radio checked" in {
            ...
            WsTestClient.withClient { client =>
              val result =
                createClientRequestGet(client, correctReturnBaseUrl + normalRoutePath(indexOfWarehouseToBeRemoved))
              ...
              whenReady(result) { res =>
                ...
                page
                  .getElementById("warehouseToRemove")
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(
      CorrectReturn,
      correctReturnBaseUrl + normalRoutePath(indexOfWarehouseToBeRemoved)
    )
  }
  ...
  s"GET " + checkRoutePath(indexOfWarehouseToBeRemoved) - {
      ...
      case (key, userAnswers) =>
        s"when the userAnswers contains data for the page with " + key + " selected" - {
          s"should return OK and render the page without the " + key + " radio checked" in {
            ...
            WsTestClient.withClient { client =>
              val result =
                createClientRequestGet(client, correctReturnBaseUrl + checkRoutePath(indexOfWarehouseToBeRemoved))
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(
      CorrectReturn,
      correctReturnBaseUrl + checkRoutePath(indexOfWarehouseToBeRemoved)
    )
  }
  ...
  s"POST " + normalRoutePath(indexOfWarehouseToBeRemoved) - {
      ...
      "when the user selects " + key - {
        "should redirect to the Secondary warehouse details controller" - {
          "when the session contains no data for page" in {
            ...
            setUpForCorrectReturn(
              completedUserAnswersForCorrectReturnNewPackerOrImporter.copy(warehouseList =
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(
      CorrectReturn,
      correctReturnBaseUrl +
        normalRoutePath(indexOfWarehouseToBeRemoved),
      Some(Json.obj("value" -> "true"))
    )
  }
  ...
  s"POST " + checkRoutePath(indexOfWarehouseToBeRemoved) - {
      ...
      "when the user selects " + key - {
        "should redirect to the Secondary Warehouse controller " - {
          "when the session contains no data for page" in {
            ...
            setUpForCorrectReturn(
              emptyUserAnswersForCorrectReturn.copy(warehouseList =
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(
      CorrectReturn,
      correctReturnBaseUrl +
        checkRoutePath(indexOfWarehouseToBeRemoved),
      Some(Json.obj("value" -> "true"))
    )
  }
}
