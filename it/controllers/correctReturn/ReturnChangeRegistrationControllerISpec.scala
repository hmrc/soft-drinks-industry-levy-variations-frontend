...
class ReturnChangeRegistrationControllerISpec extends ControllerITTestHelper {
  ...
  val userAnswersNewImporterOnly: UserAnswers =
    completedUserAnswersForCorrectReturnNewPackerOrImporter.set(PackagedAsContractPackerPage, false).success.value
      ...
      WsTestClient.withClient { client =>
        ...
        whenReady(result1) { res =>
          ...
          page.title must include(
            messages("You changed your soft drinks business activity - Soft Drinks Industry Levy - GOV.UK")
          )
        }
      }
    }
      ...
      WsTestClient.withClient { client =>
        ...
        whenReady(result) { res =>
          ...
          res.header(HeaderNames.LOCATION) mustBe Some(
            controllers.correctReturn.routes.PackAtBusinessAddressController.onPageLoad(NormalMode).url
          )
        }
      }
    }
    ...
    "when user is only a new importer should redirect to the Ask Secondary Warehouse Controller" in {
      ...
      WsTestClient.withClient { client =>
        ...
        whenReady(result) { res =>
          res.status mustBe 303
          res.header(HeaderNames.LOCATION) mustBe Some(
            controllers.correctReturn.routes.AskSecondaryWarehouseInReturnController.onPageLoad(NormalMode).url
          )
        }
      }
    }
