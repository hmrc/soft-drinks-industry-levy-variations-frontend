...
class RemovePackagingSiteConfirmControllerISpec extends ControllerITTestHelper {
  ...
  "GET " + normalRoutePath("indexDoesntExist") - {
    ...
    userAnswersForCorrectReturnRemovePackagingSiteConfirmPage(indexOfPackingSiteToBeRemoved).foreach {
      case (key, userAnswers) =>
        s"when the userAnswers contains data for the page with " + key + " selected" - {
          s"should return OK and render the page without the " + key + " radio checked" in {
            ...
            WsTestClient.withClient { client =>
              val result =
                createClientRequestGet(client, correctReturnBaseUrl + normalRoutePath(indexOfPackingSiteToBeRemoved))
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(
      CorrectReturn,
      correctReturnBaseUrl + normalRoutePath(indexOfPackingSiteToBeRemoved)
    )
  }
  ...
  s"GET " + checkRoutePath(indexOfPackingSiteToBeRemoved) - {
      ...
      case (key, userAnswers) =>
        s"when the userAnswers contains data for the page with " + key + " selected" - {
          s"should return OK and render the page without the " + key + " radio checked" in {
            ...
            WsTestClient.withClient { client =>
              val result =
                createClientRequestGet(client, correctReturnBaseUrl + checkRoutePath(indexOfPackingSiteToBeRemoved))
    ...
    testAuthenticatedWithUserAnswersForUnsupportedJourneyType(
      CorrectReturn,
      correctReturnBaseUrl + checkRoutePath(indexOfPackingSiteToBeRemoved)
    )
  }
  ...
  s"POST " + normalRoutePath(indexOfPackingSiteToBeRemoved) - {
    ...
    List(true, false).foreach { lastPackagingSite =>
      userAnswersForCorrectReturnRemovePackagingSiteConfirmPage(indexOfPackingSiteToBeRemoved, lastPackagingSite)
                ...
                WsTestClient.withClient { client =>
                  ...
                  whenReady(result) { res =>
                    ...
                    val dataStoredForPage =
                      userAnswersAfterTest.fold[Option[Boolean]](None)(_.get(RemovePackagingSiteConfirmPage))
                    if (yesSelected) {
