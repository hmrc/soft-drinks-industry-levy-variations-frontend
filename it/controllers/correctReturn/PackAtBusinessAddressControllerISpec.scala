...
class PackAtBusinessAddressControllerISpec extends ControllerITTestHelper with TryValues {
  ...
  "GET " + normalRoutePath - {
    ...
    "user selected no, user should be taken to ALF in normal mode" in {
      val journeyConfigToBePosted: JourneyConfig = JourneyConfig(
        ...
        options = JourneyOptions(
          ...
          phaseFeedbackLink = Some(
            s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-variations-frontend&backUrl=http%3A%2F%2Flocalhost%3A8705%2Fsoft-drinks-industry-levy-variations-frontend%2Fcorrect-return%2Fpack-at-business-address"
          ),
      ...
      WsTestClient.withClient { client =>
        val result1 =
          createClientRequestPOST(client, correctReturnBaseUrl + normalRoutePath, Json.obj("value" -> "false"))
