      ...
      val journeyConfigToBePosted: JourneyConfig = JourneyConfig(
        ...
        options = JourneyOptions(
          ...
          phaseFeedbackLink = Some(
            s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-variations-frontend&backUrl=http%3A%2F%2Flocalhost%3A8705%2Fsoft-drinks-industry-levy-variations-frontend%2Fcorrect-return%2Fpackaging-site-details"
          ),
        ...
        "when the session contains no data for page and is new importer" in {
          build
            .commonPreconditionChangeSubscription(
              diffSubscription.copy(activity = diffSubscription.activity.copy(importer = false))
            )
          val userAnswers = emptyUserAnswersForCorrectReturn
          ...
          WsTestClient.withClient { client =>
            ...
            whenReady(result) { res =>
              ...
              res.header(HeaderNames.LOCATION) mustBe Some(
                routes.AskSecondaryWarehouseInReturnController.onPageLoad(NormalMode).url
              )
              val dataStoredForPage =
                getAnswers(userAnswers.id).fold[Option[Boolean]](None)(_.get(PackagingSiteDetailsPage))
              dataStoredForPage.isEmpty mustBe true
  ...
  s"POST " + checkRoutePath - {
    "when user selected yes, user should be taken to ALF" in {
      val journeyConfigToBePosted: JourneyConfig = JourneyConfig(
        ...
        options = JourneyOptions(
          continueUrl =
            s"http://localhost:8705/soft-drinks-industry-levy-variations-frontend/off-ramp/change-packing-site-details/$sdilNumber",
          homeNavHref = None,
          signOutHref = Some(controllers.auth.routes.AuthController.signOut().url),
          accessibilityFooterUrl =
            Some("localhost/accessibility-statement/soft-drinks-industry-levy-variations-frontend"),
          phaseFeedbackLink = Some(
            s"http://localhost:9250/contact/beta-feedback?service=soft-drinks-industry-levy-variations-frontend&backUrl=http%3A%2F%2Flocalhost%3A8705%2Fsoft-drinks-industry-levy-variations-frontend%2Fcorrect-return%2Fchange-packaging-site-details"
          ),
          deskProServiceName = None,
          showPhaseBanner = Some(false),
          alphaPhase = Some(false),
