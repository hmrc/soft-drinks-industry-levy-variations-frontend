...
object checkReturnsVariationSubmission {
  ...
  def checkReturnVariationSubmissionSent(wireMockServer: WireMockServer,
                                      expectedReturn: SdilReturn,
                                      ): Boolean = {
    val requestToSendReturn = wireMockServer.getAllServeEvents.asScala.toList.map(_.getRequest).filter(_.getUrl.contains("/returns/vary/")).head
