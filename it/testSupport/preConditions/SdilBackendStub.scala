...
case class SdilBackendStub()(implicit builder: PreconditionBuilder) {
  def returns_pending(utr: String) = {
    stubFor(
      get(urlPathMatching(s"/returns/$utr/pending"))
        .willReturn(ok(Json.toJson(returnPeriods).toString()))
    )
    builder
  }
  ...
  def returns_variable(utr: String, returnPeriods: List[ReturnPeriod] = returnPeriodList) = {
    stubFor(
      get(urlPathMatching(s"/returns/$utr/variable"))
        .willReturn(ok(Json.toJson(returnPeriods).toString()))
    )
    builder
  }
  ...
  def retrieveSubscriptionToModify(
    identifier: String,
    refNum: String,
  ...
  def checkSmallProducerStatus(
    ...
    stubFor(
      get(urlPathMatching(s"/subscriptions/sdil/$sdilRef/year/${period.year}/quarter/${period.quarter}"))
  ...
  def balance(sdilRef: String, withAssessment: Boolean, balance: BigDecimal = BigDecimal(1000)) = {
    stubFor(
      get(urlPathMatching(s"/balance/$sdilRef/$withAssessment"))
        .willReturn(ok(Json.toJson(balance).toString()))
    )
    builder
  }
  ...
  def balanceHistory(sdilRef: String, withAssessment: Boolean, finincialItems: List[FinancialLineItem]) = {
    stubFor(
      get(urlPathMatching(s"/balance/$sdilRef/history/all/$withAssessment"))
        .willReturn(ok(Json.toJson(finincialItems).toString()))
    )
    builder
  }
  ...
  def submitVariationSuccess(sdilRef: String) = {
    stubFor(
      post(urlPathMatching(s"/submit-variations/sdil/$sdilRef"))
        .willReturn(noContent())
    )
    builder
  }

  def submitSdilReturnsVary(sdilRef: String) = {
    stubFor(
      post(urlPathMatching(s"/returns/vary/$sdilRef"))
        .willReturn(noContent())
    )
    builder
  }

  def submitReturnVariations(sdilRef: String): PreconditionBuilder = {
    stubFor(
